...
CREATE
OR REPLACE PROCEDURE generate_discovery_report(
	IN jobrunid uuid,
	IN schema_name TEXT
)
 LANGUAGE plpgsql
...
create
temp table temp_categorized_files as
select case
...
into
    temp_json
from (select size_group,
             COUNT(*) as file_count
      from temp_categorized_files
      group by size_group) as file_metrics;

aggregated_json
:= aggregated_json || temp_json;
...
into
    temp_json
from (select modified_group as date_group,
             COUNT(*)       as file_count
      from (select case
                       WHEN modified_time > CURRENT_DATE THEN 'future'
                       WHEN modified_time >= CURRENT_DATE -
                interval '1 week' THEN '0-1 wk'
                ...
                WHEN modified_time
                 < CURRENT_DATE - interval '10 year' THEN '10+ yr'
                ELSE 'invalid'
                end as modified_group
                 , file_size
            from
                temp_categorized_files) as date_metrics
      group by modified_group) as modified_date_metrics;

aggregated_json
:= aggregated_json || temp_json;
   raise
notice 'Aggregated Data: second %',
aggregated_json;
...
into
    temp_json
from (select created_group as date_group,
             COUNT(*)      as file_count
      from (select case
                       WHEN creation_time > CURRENT_DATE THEN 'future'
                       WHEN creation_time >= CURRENT_DATE -
                interval '1 week' THEN '0-1 wk'
            ...
            from
                temp_categorized_files) as date_metrics
      group by created_group) as created_date_metrics;
...
into
    temp_json
from (select SUBSTRING(file_name FROM '\.([^\.]+)$') as file_extension,
             COUNT(*)                                as file_count,
             SUM(file_size)                          as total_size
      from temp_categorized_files
      where is_directory = false
      group by file_extension
      order by total_size desc) as extension_metrics;
...
from (select case
      ...
      from temp_categorized_files
      group by depth_group) as depth_metrics;
...
into
    temp_json
from (select SUM(depth) as sunOfDepth
      from temp_categorized_files) as depth_metrics;
...
from (select size_group,
             ...
             SUM(file_size) as total_space
      from temp_categorized_files
      group by size_group) as file_space_metrics;
...
into
    temp_json
from (select 'Total Count' as sub_category,
      ...
      from temp_categorized_files
      union all
      select 'Total Space for Regular Files',
             SUM(case when is_directory = false then file_size else 0 end),
             'size' as valueType
      from temp_categorized_files
      union all
      select 'Total Space for Directories',
      ...
      select 'Total Space Used',
             SUM(file_size),
             'size' as valueType
      from temp_categorized_files) as stats;
...
into
    temp_json
from (select 'max_file_size' as stat_type,
             MAX(case when is_directory = false then file_size else null end)::numeric as stat_value, 'size' as valueType
      from temp_categorized_files
      union all
      select 'max_name_length' as stat_type,
             MAX(case when is_directory = false then LENGTH(file_name) else null end)::numeric as stat_value, 'length' as valueType
      from temp_categorized_files
      union all
      select 'total_directories' as stat_type,
             SUM(case when is_directory = true then 1 else 0 end)::numeric as stat_value, 'count' as valueType
      from temp_categorized_files) as stats;
...
into temp_json
FROM (SELECT 'Total Time' AS stat_type,
             COALESCE(EXTRACT(EPOCH FROM (jr.end_time - jr.start_time)) ::INT,
                      0)::TEXT AS stat_value, 'time' as valueType
      FROM jobrun jr
      WHERE jr.id = jobrunid

      UNION ALL

      SELECT 'Status'  AS stat_type,
             jr.status AS stat_value,
             'status'  as valueType
      FROM jobrun jr
      WHERE jr.id = jobrunid) AS job_run_stats;
...
into temp_json
FROM (SELECT 'Path' AS sub_category, v.volume_path AS value
      FROM config c
          JOIN file_server fs2
      ON fs2.config_id = c.id
      ...
      ON fs2.config_id = c.id
          JOIN volume v ON v.file_server_id = fs2.id
      WHERE v.id in (select path_id from temp_categorized_files limit 1 )) AS file_server_data;
...
from (select jsonb_build_object(
      ...
      from (select STRING_AGG(file_name || ' (' || length(file_name) || ')',
                              '; ') as longest_file_names
            from (select file_name,
                         length(file_name)
                  from inventory i
                  where i.is_directory is false
                  order by length(file_name) desc limit 5) subquery) combined_results
      union all
      ...
      from (select COUNT(path)       as path_count,
                   SUM(length(path)) as total_length
            from inventory i
            where i.is_directory is false) as summary_result
      ...
      from (select STRING_AGG(file_name || ' (' || length(file_name) || ')',
            ...
            from (select file_name,
                         length(file_name)
                  from inventory i
                  where i.is_directory is true
                  order by length(file_name) desc limit 5) subquery) as combined_result
      union all
      select jsonb_build_object(
                     'category',
                     'Biggest',
      ...
      from (select STRING_AGG(path || ' (' || length(path) || ')',
            ...
            from (select path,
                  ...
                  where i.is_directory is false
                  group by path
                  order by length(path) desc limit 5) subquery) as combined_result
      union all
      select jsonb_build_object(
                     'category',
                     'Biggest',
      ...
      from (select STRING_AGG(file_name || ' (' || file_size || ')',
            ...
            from (select file_name,
                  ...
                  group by file_name,
                           file_size
                  order by file_size desc limit 5) subquery) as combined_result
      union all
      select jsonb_build_object(
                     'category',
                     'Biggest',
      ...
      from (select STRING_AGG(directory || ' (' || file_count || ')',
                              '; ') as biggest_directory_count
            from (select parent_path as directory,
                         COUNT(*)    as file_count
                  from inventory
                  where parent_path is not null
                    and is_directory is true
                  group by parent_path
                  order by file_count desc limit 5) subquery) as combined_result
      union all
      select jsonb_build_object(
                     'category',
                     'Biggest',
      ...
      from (select STRING_AGG(directory || ' (' || total_size || ')',
            ...
            from (select parent_path    as directory,
                  ...
                  where parent_path is not null
                    and is_directory is true
                  group by parent_path
                  order by total_size desc limit 5) subquery) as combined_result) result;
