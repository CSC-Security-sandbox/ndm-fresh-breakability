-- Rollback: restore job_stats_summary_mv as defined in 20260313_02 (newly/recopied include directory rows).
...
inventory_stats AS (
  SELECT
    ...
    COUNT(*) FILTER (WHERE (entry_type IS NULL OR entry_type = 'inventory') AND update_type = 'new' AND NOT COALESCE(is_deleted, false)) AS newly_copied_count,
    COUNT(*) FILTER (
