...
func CreateADPrincipals(users []string, groups string) error {
	// Parse comma-separated groups string into slice
	var groupSlice []string
	if groups != "" {
		groupSlice = strings.Split(groups, ",")
	}
	script := createADPrincipalsScript(users, groupSlice)
...
func createADPrincipalsScript(users []string, groups []string) string {
...
func createSMBFilesWithMixedPrincipalsScript(export string, validUsers, invalidUsers, validGroups, invalidGroups []string) string {
	...
	// Apply permissions for valid groups using Get-ADGroup
	if len(validGroups) > 0 {
		groupname := strings.Split(validGroups[0], `\`)[1]
		script += fmt.Sprintf(`try { $group1 = Get-ADGroup -Identity '%s' -Credential $credential; `, groupname)
		script += `$sidg1 = New-Object System.Security.Principal.SecurityIdentifier($group1.SID); `
	...
	// Apply permissions for invalid groups using Get-ADGroup
	if len(invalidGroups) > 0 {
		groupname := strings.Split(invalidGroups[0], `\`)[1]
		script += fmt.Sprintf(`try { $igroup1 = Get-ADGroup -Identity '%s' -Credential $credential; `, groupname)
		script += `$isidg1 = New-Object System.Security.Principal.SecurityIdentifier($igroup1.SID); `
...
func createSMBFilesForSIDMappingScript(export string, users []string) string {
	...
	if len(users) > 2 {
		...
		script += fmt.Sprintf(`try { `+
			`$principal3 = Get-ADUser -Identity '%s' -Credential $credential; `+
			`$sid3 = New-Object System.Security.Principal.SecurityIdentifier($principal3.SID); `+
...
// GetADUserSID retrieves the SID for a given AD user
func GetADUserSID(username string) (string, error) {
	...
	script := getADUserSIDScript(username)
	...
	output, err := sshRunScript(sshConfig, script)
	if err != nil {
		return "", fmt.Errorf("GetADUserSID failed: %w\noutput: %s", err, output)
	}

	// Parse output to extract SID (format: "SID: S-1-5-21-...")
	...
	for _, line := range lines {
		...
		if strings.HasPrefix(line, "SID: ") {
			...
			if strings.HasPrefix(sid, "S-1-5-21-") {
				LogDebug(fmt.Sprintf("Found SID for user %s: %s", username, sid))
				return sid, nil
			}
		}
	}

	return "", fmt.Errorf("failed to find SID for user %s in output: %s", username, output)
}

func getADUserSIDScript(username string) string {
	adUsername := PROTOCOL_USERNAME
	adPassword := PROTOCOL_PASSWORD
	...
	parts = append(parts, fmt.Sprintf(`$credential = New-Object System.Management.Automation.PSCredential('%s', $password); `, adUsername))
	parts = append(parts, `Import-Module ActiveDirectory -ErrorAction Stop; `)
	parts = append(parts, fmt.Sprintf(`$user = Get-ADUser -Identity '%s' -Credential $credential -ErrorAction Stop; `, username))
	parts = append(parts, `Write-Host "SID: $($user.SID)"; `)
	parts = append(parts, `"`)
