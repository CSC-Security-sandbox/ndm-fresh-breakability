...
// parseMigrationDirs splits a comma-separated env value into trimmed directory names.
func parseMigrationDirs(raw string) []string {
	var dirs []string
...
func UpdateConfVariables(protocolType, environment string) {
	...
	case ProtocolNFS:
		...
		MIGRATION_DIRS = parseMigrationDirs(os.Getenv("MIGRATION_NFS_DIR"))
	...
	case ProtocolSMB:
		...
		MIGRATION_DIRS = parseMigrationDirs(os.Getenv("MIGRATION_SMB_DIR"))
		if len(MIGRATION_DIRS) > 0 {
			MIGRATION_DIR = MIGRATION_DIRS[0]
		}
