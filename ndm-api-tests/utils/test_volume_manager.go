...
func (tm *TestVolumeManager) CreateCloneVolume(masterVolumeName string) (string, error) {
	...
	if PROTOCOL_TYPE == ProtocolNFS {
		...
		if err != nil {
			...
			// Give NFS server time to register and propagate the new export
			// This ensures showmount -e and file server refresh will return the new export
			LogDebug(fmt.Sprintf("Waiting for NFS export to propagate for volume '%s'...", cloneName))
