...
func FetchIsilonCertificate(host string, headers map[string]string) (*CertificateResponse, error) {
	...
	LogDebug(fmt.Sprintf("Certificate fetched successfully. Expires: %s", certResp.Data.ValidTo))
	return &certResp, nil
}
...
func CreateIsilonFileServerWithFlow(
	...
	LogDebug(fmt.Sprintf("Certificate fetched successfully. Valid to: %s", certResp.Data.ValidTo))
	...
	params := CreateIsilonServerParams{
		...
		TlsCertificate:     certResp.Data.CertificatePEM,
		TlsExpiry:          certResp.Data.ValidTo,
		TlsAccepted:        true,
		Zones:              zones,
	}
