...
run_tests() {
    ...
    # eg reports/2025-07-22/SMB/end-to-end-SMB-1753166831.log
    ...
    local clone_provider="${VOLUME_CLONE_PROVIDER:-gcnv}"
...
if [[ "$RUN_SMOKE" == true ]]; then
    ...
    run_tests "smoke" "./tests/smoke" "GCP" "SMB"
fi

#End-to-End Testing
if [[ "$RUN_E2E" == true ]]; then
    echo "Running SMB End-to-End Tests..."
    run_tests "end-to-end" "./tests/e2e" "GCP" "SMB"
fi

#Regression Testing
if [[ "$RUN_REGRESSION" == true ]]; then
    echo "Running SMB Regression Tests..."
    run_tests "regression" "./tests/regression" "GCP" "SMB"
fi
