...
// - Background metrics collection during polling
//
// Parameters:
...
// - label: Label for metrics collection during polling
// - cpIP: Control plane IP for metrics collection (optional)
// - workerIDs: Worker IDs for metrics collection (optional)
//
// Returns error if any job fails or if too many consecutive API errors occur.
func pollJobRunsInfinite(jobConfigIDs []string, jobType string, headers map[string]string, pollInterval time.Duration, label string, cpIP string, workerIDs []string) error {
	...
	defer cancelMetrics() // Ensure metrics collection stops when function exits

	go func() {
		metricsInterval := 5 * time.Minute // Collect metrics every 5 minutes
		...
		for {
			select {
			case <-ctx.Done():
				log.Printf("[%s] Stopping background metrics collection for label: %s", jobType, label)
				return
			case <-ticker.C:
				log.Printf("[%s] Collecting metrics for label: %s (interval #%d)", jobType, label, intervalCount)

				// Collect metrics for all workers if available
				if cpIP != "" && len(workerIDs) > 0 {
					for i, workerID := range workerIDs {
						metricsLabel := fmt.Sprintf("%s_INTERVAL_%d_WORKER_%d", label, intervalCount, i+1)
						err := collectWorkerMetricsWithLabel(cpIP, workerID, metricsLabel)
						if err != nil {
							log.Printf("[%s] Warning: Failed to collect metrics for worker %s: %v", jobType, workerID, err)
						}
					}
				} else {
					log.Printf("[%s] Skipping metrics collection - missing CP IP or worker IDs", jobType)
				}
...
func runNDMScaleTest(cpImageName, workerImageName, instancePrefix string) error {
	...
	for i, workerID := range workerIDs {
		err = collectWorkerMetricsWithLabel(cpIP, workerID, fmt.Sprintf("MIGRATION_WORKER_%d", i+1))
		if err != nil {
			LogFatalf("Error collecting worker metrics for %s: %v", workerID, err)
		}
	}
	...
	err = pollJobRunsInfinite(migrationJobConfigIDs, "MIGRATION", headers, pollInterval, "migration-scale-test", cpIP, workerIDs)
	if err != nil {
	...
	err = pollJobRunsInfinite(jobConfigIDs, "CUTOVER", headers, cutoverPollInterval, "cutover-scale-test", cpIP, workerIDs)
	if err != nil {
