...
var CSV_REPORT_HEADERS = []string{"Pack", "Worker-Host", "Protocol", "Source-FileServer", "Destination-FileServer",
	...
	"Migration-Duration", "Line-Rate", "Worker-Downtime (mins)"}
	...
	Context("TC-001", func() {
		...
		It("TC-001: Run performance packs with same migration config", func() {
			...
			for packNumb := 1; packNumb <= NUMBER_OF_PACKS; packNumb++ {
				err = UpdateWorkerConfig(PERF_PACK_CONFIG[packNumb]["MAX_WRITE_CONCURRENCY"], PERF_PACK_CONFIG[packNumb]["JOB_TASK_ACTIVITY_CONCURRENCY"],
				...
				for run := 1; run <= MIGRATIONS_PER_PACK; run++ {
					_, err := IsWorkerRunning()
					Expect(err).NotTo(HaveOccurred(), fmt.Sprintf("failed to get worker status, pack=%d, err=%v", packNumb, err))
					...
					migrationDuration, lineRate, err := getMigrationDurationAndLineRate(jobRunInfo.StartTime, jobRunInfo.EndTime)
		...
		AfterEach(func() {
			...
			err = UpdateWorkerConfig(PERF_PACK_CONFIG[1]["MAX_WRITE_CONCURRENCY"], PERF_PACK_CONFIG[1]["JOB_TASK_ACTIVITY_CONCURRENCY"],
...
func getMigrationDurationAndLineRate(startTime, endTime string) (string, string, error) {
...
func appendRowsToPerfCSV(packNumb int, jobRunID, migrationDuration, lineRate string, emptyRowsNumb ...int) error {
