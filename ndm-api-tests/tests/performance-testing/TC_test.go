...
var CSV_REPORT_HEADERS = []string{"Pack", "Worker-Host", "Protocol", "Source-FileServer", "Destination-FileServer",
	...
	"Migration-Duration", "Throughput", "Worker-Downtime", "Worker-Max-CPU-Usage"}
		...
		It("TC-001: Run performance packs with same migration config", func() {
			...
			for packNumb := 1; packNumb <= NUMBER_OF_PACKS; packNumb++ {
				...
				for run := 1; run <= MIGRATIONS_PER_PACK; run++ {
					...
					migrationDuration, throughput, err := getMigrationDurationAndThroughput(jobRunInfo.StartTime, jobRunInfo.EndTime)
					Expect(err).NotTo(HaveOccurred(), fmt.Sprintf("failed to get migration duration and throughput, pack=%d, iteration=%d, err = %v", packNumb, run, err))

					workerDownTimeMin := float64(workerDownTimeSec) / float64(60)

					err = appendRowsToPerfCSV(packNumb, jobRunID, migrationDuration, throughput, maxCPUUsageInPercentage, workerDownTimeMin)
...
func getMigrationDurationAndThroughput(startTime, endTime string) (string, string, error) {
	...
	// Calculate throughput using complete migration time
	throughput := size / totalDuration

	return fmt.Sprintf("%dh%02dmin", hours, minutes), fmt.Sprintf("%.4f %s/sec", throughput, unit), nil
}

func appendRowsToPerfCSV(packNumb int, jobRunID, migrationDuration, throughput, maxCPUUsage string, workerDownTimeMin float64, emptyRowsNumb ...int) error {
	...
	newRows := [][]string{
		{
			...
			migrationDuration, throughput, fmt.Sprintf("%.4f mins", workerDownTimeMin),
			maxCPUUsage,
		},
	}
