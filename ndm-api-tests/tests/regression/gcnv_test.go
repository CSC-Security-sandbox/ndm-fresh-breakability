...
var _ = Describe("GCNV Flex Test regression", Ordered, func() {
	...
	Context("Setup", func() {
		...
		It("Should return error while uploading an empty path file to the file server", func() {
			...
			Expect(uploadStats.Message).To(Equal("Error processing file upload: CSV file is empty or does not contain valid data. The first line should contain \"path\" as the header."))
		...
		It("Should return error while uploading a file with headers only", func() {
			...
			Expect(err).NotTo(HaveOccurred(), "Error uploading file with headers only")
			defer resp.Body.Close()
			Expect(uploadStats.Message).To(Equal("Error processing file upload: CSV file is empty or does not contain valid export paths."))
			Expect(uploadStats.UploadId).To(BeEmpty(), "Expected empty upload ID for empty file")
		})
		...
		It("Should upload a path file with a single valid path to the file server", func() {
			...
			Expect(fileServerDetails.FileServers[0].Volumes[0].VolumePath).To(Equal(NFS_SOURCE_VOLUME_1), "Expected volume export path to match the uploaded path")
		...
		It("Should reupload a path file with multiple paths to the file server", func() {
			...
			defer resp.Body.Close()
			Expect(uploadStats.UploadId).NotTo(BeEmpty(), "Expected non-empty upload ID")
			Expect(uploadStats.NewPaths).To(BeNumerically("==", 0), "Expected one new path to be uploaded")
			Expect(uploadStats.AlreadyExistingPaths).To(BeNumerically("==", 2), "Expected no already existing paths")
			Expect(uploadStats.NoLongerAvailablePaths).To(BeNumerically("==", 0), "Expected one path to be no longer available")
			Wait(10)
		...
		It("Should reupload a path file with 1 new valid path", func() {
			...
			Expect(uploadStats.NewPaths).To(BeNumerically("==", 1), "Expected new path to be uploaded")
			Expect(uploadStats.AlreadyExistingPaths).To(BeNumerically("==", 0), "Expected no already existing paths")
			Expect(uploadStats.NoLongerAvailablePaths).To(BeNumerically("==", 2), "Expected no paths to be no longer available")
			Wait(10)
			...
			fileServerDetails, err := GetFileServerDetails(SourceConfigID, headers)
			Expect(err).NotTo(HaveOccurred(), "Error sending get file server details API request")
			Expect(len(fileServerDetails.FileServers[0].Volumes)).To(BeNumerically("==", 3), "Expected one volumes to be present in the file server")

			// GetVolumeDetailsFromFileServer for the invalid path
			By("Getting volume details for the valid path")
			validPath1, err := GetVolumeDetailsFromFileServer(fileServerDetails.FileServers[0].Volumes, NFS_SOURCE_VOLUME_1)
			Expect(err).NotTo(HaveOccurred(), "Expected to find volume path")
			...
			validPath2, err := GetVolumeDetailsFromFileServer(fileServerDetails.FileServers[0].Volumes, NFS_SOURCE_VOLUME)
			Expect(err).NotTo(HaveOccurred(), "Expected to find volume path")
		...
		It("Should reupload a path file with 1 valid path and 1 invalid path", func() {
			...
			fileContent := FileContent{
				FileName: "test_single_path_file.csv",
				FileSize: 1024,
				Contents: fmt.Sprintf("path\n%s\n/srv/invalid_share1", NFS_SOURCE_VOLUME),
			}
			...
			Expect(uploadStats.UploadId).NotTo(BeEmpty(), "Expected non-empty upload ID")
			Expect(uploadStats.NewPaths).To(BeNumerically("==", 1), "Expected new path to be uploaded")
			Expect(uploadStats.AlreadyExistingPaths).To(BeNumerically("==", 1), "Expected no already existing paths")
			Expect(uploadStats.NoLongerAvailablePaths).To(BeNumerically("==", 2), "Expected no paths to be no longer available")
			Wait(10)
			...
			fileServerDetails, err := GetFileServerDetails(SourceConfigID, headers)
			Expect(err).NotTo(HaveOccurred(), "Error sending get file server details API request")
			Expect(len(fileServerDetails.FileServers[0].Volumes)).To(BeNumerically("==", 4), "Expected one volumes to be present in the file server")

			// GetVolumeDetailsFromFileServer for the valid path
			By("Getting volume details for the valid path")
			validPath1, err := GetVolumeDetailsFromFileServer(fileServerDetails.FileServers[0].Volumes, NFS_SOURCE_VOLUME)
			Expect(err).NotTo(HaveOccurred(), "Expected to find volume path")
			Expect(validPath1.IsValid).To(BeTrue(), "Expected volume to be valid")
			...
			By("Getting volume details for the NFS_SOURCE_VOLUME_1 path")
			validPath2, err := GetVolumeDetailsFromFileServer(fileServerDetails.FileServers[0].Volumes, NFS_SOURCE_VOLUME_1)
			Expect(err).NotTo(HaveOccurred(), "Expected to find volume path")
			Expect(validPath2.IsValid).To(BeTrue(), "Expected volume to be valid")
