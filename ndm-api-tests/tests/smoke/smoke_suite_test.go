...
var _ = SynchronizedBeforeSuite(func() []byte {
	...
	LogDebug(fmt.Sprintf("[Smoke Process #1] Serialized shared data (%d bytes) for distribution to parallel nodes", len(data)))
	return data
}, func(data []byte) {
	// This runs on ALL processes (including Process #1)
	By("Initializing smoke test environment variables")
	flag.Parse()
	UpdateConfVariables(ProtocolType, Environment)
	...
	LogDebug(fmt.Sprintf("[Smoke All Processes] Received shared project: %s (ID: %s) with %d workers",
		GlobalProjectName, GlobalProjectId, len(GlobalAttachedWorkersConfig)))
})
...
var _ = SynchronizedAfterSuite(func() {
	...
	if len(GlobalAttachedWorkersConfig) > 0 {
		...
		LogDebug("Smoke suite cleanup complete")
	}
})