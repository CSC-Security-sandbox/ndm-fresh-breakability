...
var _ = Describe("TC-003: Create a fileserver with healthy workers and run scheduled discovery and migration", func() {
	...
	It("TC-003: Create a fileserver with healthy workers and run scheduled discovery and migration", func() {
		...
		var jobConfigID, jobConfigID1, cutoverRunID, cutoverRunID1 string
		var destinationConfigID, destinationPathID, destinationPathID1 string
		var destinationJobConfigID, destinationJobConfigID1, destinationDiscoveryJobRunID, destinationDiscoveryJobRunID1 string
		...
		sourceFutureDateTime := time.Now().UTC().
			Add(2 * time.Minute).
		...
		jobParams := DiscoveryJobParams{
			...
			FirstRunAt:               sourceFutureDateTime,
			CreatedBy:                nil,
		...
		getJobsResp, resp, err := GetJobRunDetails(sourceConfigID, headers, false)
		...
		getJobsResp, resp, err = GetJobRunDetails(sourceConfigID1, headers, false)
		Expect(getJobsResp.JobRuns).To(BeEmpty(), "Expected jobRuns to be empty")
		Expect(getJobsResp.Status).To(Equal("ACTIVE"), "Expected status to be ACTIVE")
		Expect(getJobsResp.JobType).To(Equal("DISCOVER"), "Expected jobType to be DISCOVER")
		Expect(err).NotTo(HaveOccurred(), "Error getting job run ID")
		...
		destinationFutureDateTime := time.Now().UTC().
			Add(2 * time.Minute).
			Format("2006-01-02T15:04:05.000Z")
		...
		destinationJobParams := DiscoveryJobParams{
			...
			ExcludeFilePatterns:      "",
			PreserveAccessTime:       false,
			FirstRunAt:               destinationFutureDateTime,
			CreatedBy:                nil,
			WorkflowExecutionTimeout: "60s",
			WorkflowTaskTimeout:      "30s",
			WorkflowRunTimeout:       "30s",
		...
		migrationParams := MigrationJobParams{
			FirstRunAt:         migrationFutureDateTime,
			FutureRunSchedule:  "",
			SourcePathIDs:      []string{sourcePathID, sourcePathID1},
