...
var _ = Describe("TC-008: Run migration with 'Skip files modified in last' option", func() {
	...
	Context("TC-008", func() {
		...
		It("TC-008: Run migration with 'Skip files modified in last' option", func() {
			...
			err = ModifyDataOnVolume(sourceVolumePath1)
			Expect(err).NotTo(HaveOccurred(), "Error while modify volume of %s", sourceVolumePath1)
			err = ModifyDataOnVolume(sourceVolumePath2)
			Expect(err).NotTo(HaveOccurred(), "Error while modify volume of %s", sourceVolumePath2)
		...
		AfterEach(func() {
			...
			Expect(err).NotTo(HaveOccurred(), "Error while clearing volume of %s", sourceVolumePath1)
			...
			Expect(err).NotTo(HaveOccurred(), "Error while restoring volume of %s", destinationVolumePath1)
