...
func initIsilonEnv() {
	...
	if portStr := os.Getenv("ISILON_MANAGEMENT_PORT"); portStr != "" {
		ISILON_MANAGEMENT_PORT, _ = strconv.Atoi(portStr)
	}
	...
	ISILON_NFS_USERNAME = os.Getenv("AZURE_NFS_PROTOCOL_USERNAME")
	ISILON_NFS_PASSWORD = os.Getenv("AZURE_NFS_PROTOCOL_PASSWORD")
	ISILON_SMB_HOST = os.Getenv("ISILON_SMB_HOST")
	ISILON_SMB_USERNAME = os.Getenv("AZURE_SMB_PROTOCOL_USERNAME")
	ISILON_SMB_PASSWORD = os.Getenv("AZURE_SMB_PROTOCOL_PASSWORD")
...
var _ = Describe("TC-ISILON: Dell Isilon File Server Creation and Validation", func() {
	...
	Context("Dell Isilon File Server Operations", func() {
		...
		It("TC-ISILON: Complete Dell Isilon file server workflow - certificate, zones, NFS, SMB, dual-protocol, and error handling", func() {
			...
			Expect(certResp.Data.Items.CertificatePEM).NotTo(BeEmpty(), "Certificate PEM should not be empty")
			Expect(certResp.Data.Items.ValidTo).NotTo(BeEmpty(), "Certificate valid-to should not be empty")
			Expect(certResp.Data.Items.IsExpired).To(BeFalse(), "Certificate should not be expired")

			LogDebug("Certificate fetched successfully:")
			LogDebug(fmt.Sprintf("  - Fingerprint: %s", certResp.Data.Items.Fingerprint))
			LogDebug(fmt.Sprintf("  - Valid From: %s", certResp.Data.Items.ValidFrom))
			LogDebug(fmt.Sprintf("  - Valid To: %s", certResp.Data.Items.ValidTo))
			LogDebug(fmt.Sprintf("  - Days Remaining: %d", certResp.Data.Items.DaysRemaining))
			LogDebug(fmt.Sprintf("  - Is Self-Signed: %v", certResp.Data.Items.IsSelfSigned))
