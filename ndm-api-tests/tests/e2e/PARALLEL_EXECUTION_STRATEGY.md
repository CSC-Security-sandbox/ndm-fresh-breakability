...
```go
var GlobalVolumeSetup *TestVolumeSetup // Package-level singleton for configuration

func SetupTestVolumesBeforeEach() {
    if GlobalVolumeSetup == nil {
        // First test initializes the global configuration (ONTAP URLs, master volume names)
        GlobalVolumeSetup = &TestVolumeSetup{...}
        ValidateOntapConfiguration() // Verify SVMs and master volumes exist
    }
    
    // Each test creates NEW volume managers (not shared to avoid race conditions)
    sourceVolumeManager := NewTestVolumeManager(GlobalVolumeSetup.SourceOntapURL, ...)
    destVolumeManager := NewTestVolumeManager(GlobalVolumeSetup.DestOntapURL, ...)
    
    // Each test gets its own cloned volumes (isolated per test)
    sourceClones := sourceVolumeManager.CreateMultipleClones(GlobalVolumeSetup.MasterSourceVolumes)
    destClones := destVolumeManager.CreateMultipleClones(GlobalVolumeSetup.MasterDestVolumes)
    
    return sourceClones, destClones, sourceVolumeManager, destVolumeManager
}
```

**What's global vs per-test:**
- **Global (shared)**: ONTAP configuration, master volume names
- **Per-test (isolated)**: Volume managers, cloned volumes
```
...
**Before (Shared Volumes):**
```go
BeforeEach(func() {
    // All tests use the same volumes
    sourceVolumePath = "10.192.7.111:volSrcAuto"
    // ⚠️ Multiple tests writing to same volume!
})
```

**After (Cloned Volumes):**
```go
BeforeEach(func() {
    // Each test gets isolated clones
    clonedSourceVolumes, _, sourceManager, destManager, err := SetupTestVolumesBeforeEach()
    sourceVolumePath = fmt.Sprintf("%s:%s", SOURCE_HOST_IPs[0], clonedSourceVolumes[0])
...
**Configuration:**
- `.env` - Added ONTAP configuration and protocol-specific host IPs
- `.env.example` - Template for new configuration

**Core Implementation:**
- `utils/test_volume_manager.go` - Volume cloning lifecycle management
- `utils/ontap_client.go` - ONTAP REST API client, FlexClone operations
- `utils/test_setup_helper.go` - Protocol-aware volume setup and validation

**Test Updates:**
- `tests/e2e/TC-001_test.go` → TC-006_test.go - Migrated to cloning pattern
- `tests/e2e/TC-SUPPORT-BUNDLE_test.go` - Migrated to cloning pattern
- `tests/e2e/TC-SMB-REDIRECTS_test.go` - Uses AD Server volumes (no cloning)

**New Functionality:**
- SMB share creation for cloned volumes
- NFS export policy creation for cloned volumes
- Volume name generation with 10-char test case limit
- Protocol-specific host IP resolution
- AD Server volume segregation
...
## Stage 3: Test Suite Parallelization (Future Work)
...
**2. Test Suite Isolation Requirements**

Each parallel process needs:
- **Isolated Volumes**: Already solved by Stage 2 (volume cloning)
- **Isolated Workers**: Need worker pool or dynamic worker creation
- **Isolated Projects**: Need project pool or dynamic project creation
- **Resource Cleanup**: Enhanced cleanup to handle parallel failures

**3. Implementation Plan**

**Option A: Worker/Project Pools**
```go
// Pre-create pool of workers and projects
workerPool = []string{"worker-1", "worker-2", "worker-3", "worker-4"}
projectPool = []string{"project-1", "project-2", "project-3", "project-4"}

// Each parallel process claims a worker/project pair
BeforeSuite(func() {
    workerID = ClaimWorkerFromPool()
    projectID = ClaimProjectFromPool()
})
```

**Option B: Dynamic Resource Creation**
```go
// Each parallel process creates its own resources
BeforeSuite(func() {
    processID = GinkgoParallelProcess() // Returns 1, 2, 3, or 4
    workerName = fmt.Sprintf("worker-%d", processID)
    projectName = fmt.Sprintf("project-%d", processID)
...
| Challenge | Solution |
|-----------|----------|
| **Resource Conflicts** | Use process-specific naming (worker-1, worker-2, etc.) |
| **Cleanup Complexity** | Enhanced AfterSuite to cleanup process-specific resources |
| **Timing Issues** | Retry mechanisms for resource creation/deletion |
| **Log Management** | Per-process log files for debugging |
| **Shared Resources** | Identify and isolate (ONTAP API limits, Keycloak tokens) |
...
var _ = BeforeSuite(func() {
    if GinkgoParallelProcess() > 1 {
        ...
        LogDebug(fmt.Sprintf("Running in parallel mode, process %d", processID))
        
        // Create process-specific resources
        SetupProcessResources(processID)
    } else {
...
**7. Volume Cloning in Parallel Mode**
...
- Enable parallel execution for 2-3 simple tests
- Validate resource isolation
- Measure performance improvement

**Phase 2: Full Implementation**
- Create worker/project pools
- Update all tests for parallel compatibility
- Enhanced cleanup and error handling

**Phase 3: CI/CD Integration**
- Add parallel execution to GitHub Actions
- Monitor for flaky tests
- Optimize process count based on available resources
