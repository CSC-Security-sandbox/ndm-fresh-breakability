  ...
  async createInventory(
    ...
    try {
      const mappedData = data.map((item) =>
        this.mapSourceToTarget(item, jobRunId, pathId)
      );
      const inventoryRecords = this.inventoryRepo.create(mappedData);
      await this.inventoryRepo.save(inventoryRecords);
      this.logger.log(
        `Successfully inserted ${inventoryRecords.length} inventory records`
      );
    } catch (err) {
      this.logger.error(
        `Failed to save inventory records: ${err.message}`,
        err.stack
      );
      throw err;
    }
