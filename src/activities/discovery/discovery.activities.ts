...
export class DiscoveryActivity {
  ...
  async publishTask(traceId: string): Promise<any> {
    ...
    try {
      ...
      this.logger.error(
        `[${traceId}] Error in publishing task: ${error.message}`,
      );
      return {
        traceId: traceId,
        status: 'error',
  ...
  async discoveryStatusUpdate(traceId: string, status: string): Promise<any> {
    try {
      ...
      return {
        message:
          'Discovery Job status updated as completed for job id: ' + traceId,
      };
    } catch (error) {
      console.log(`[${traceId}] Failed to update discovery status: ${error}`);
      return {
        message: 'Error while updating the satus of the job id : ' + traceId,
      };
    }
  }
  async publishLastEntry(traceId: string): Promise<any> {
    try {
      ...
      this.log(traceId, `Error while marking the job as completed : ${error}`);
      return {
        message: 'Error while marking the job as completed : ' + traceId,
      };
    }
  }

    log(traceId: string, message: string) {
    console.log(`[${traceId}] ${message}`);
  }
  
}
const generateDummyFileEntry:FileInfo = 
     new FileInfo(
      "LAST_FILE", 
      "",  
      ""  ,
