...
export class RedisConsumerService implements OnModuleInit {
  ...
  async isPendingStart() {
    ...
    if (data.length > 0) {
      for (const item of data) {
        ...
        console.log(
          `Consumer ${jobRunId} ${consumerType} ${readerName} in active list`
        );
  ...
  //                 const member = this.getConsumerSetMember(jobRunId, type, '*');
  //                  console.log(member);
  ...
  //             await this.setKey(key, 'false');
  //             const member = this.getConsumerSetMember(jobRunId, consumerType, '*');
  //             await this.removeFromSet(this.activeConsumersSetKey, member);
    ...
    if (all || !consumerType) {
      ...
      for (const type of Object.values(ConsumerType)) {
        ...
        if (await this.keyExists(key)) {
          ...
          const activeMembers = await this.getSetMembers(
            this.activeConsumersSetKey
          );
      ...
      console.log(
        "Stopping specific consumer for jobRunId:",
        jobRunId,
        "and type:",
        consumerType
      );
      ...
      if (await this.keyExists(key)) {
        ...
        for (const member of activeMembers) {
          const [memberJobRunId, memberConsumerType] = member.split(":");
          if (
            memberJobRunId === jobRunId &&
            memberConsumerType === consumerType
          ) {
            await this.removeFromSet(this.activeConsumersSetKey, member);
            console.log(`Removed member: ${member}`);
          }
  ...
  //      console.log("Active Consumer Members:", activeConsumerMembers);
  //     const activeConsumers = activeConsumerMembers.map((member) => {
  ...
  async listActiveConsumers(): Promise<
    {
      jobRunId: string;
      consumerType: string;
      readerName: string;
  ...
  > {
    const activeConsumerMembers = await this.getSetMembers(
      this.activeConsumersSetKey
    );
  ...
  async startConsumerfun(
    ...
    if (!jobContext) throw new Error("jobContext is null");

    const key = this.getConsumerKey(jobRunId, consumerType);

    console.log(`[${jobRunId}] Starting consumer key: ${key}`);
    await this.setKey(key, "true");
    const member = this.getConsumerSetMember(
      jobRunId,
      consumerType,
      readerName
    );
  ...
  private async handleFiles(
    ...
    if (data.fileName === "LAST_FILE") {
      ...
      if (this.accumulatedRecords.length > 0) {
        console.log(
          `Processing remaining ${this.accumulatedRecords.length} records before stopping.`
        );
      ...
      await this.workflowService.signalWorkflow({
        ...
        input: {
          payloads: [
            defaultDataConverter.payloadConverter.toPayload(
              `${jobType}_REPORTED`
            ),
          ],
        },
    ...
    console.log(
      "this.accumulatedRecords=================",
      this.accumulatedRecords.length
    );
  ...
  async removeFromSet(setKey: string, member: string): Promise<void> {
    try {
      ...
      const removed = await this.redisClient.sendCommand([
        "SREM",
        setKey,
        member,
      ]);
