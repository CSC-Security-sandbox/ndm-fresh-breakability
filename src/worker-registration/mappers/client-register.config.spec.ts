...
describe("ClientConfig", () => {
  ...
  it("should update the projectId in protocol mappers if mapper exists", () => {
    ...
    expect(config.protocolMappers.find((m) => m.name === "custom-client-claim")?.config["claim.value"]).toBe("new-project-id");
  });

  it("should do nothing if setProjectId is called but custom-client-claim mapper is missing", () => {
    config.setProjectId("new-project-id");
    expect(config.protocolMappers.find((m) => m.name === "custom-client-claim")).toBeUndefined();
  });
  ...
  it("should return the full config when getConfig is called", () => {
    ...
    expect(configObj).toHaveProperty("clientAuthenticatorType", "client-secret");
