...
The CSV must have a `SourcePath` column. The optional `DestPath` column specifies the destination relative path (defaults to `SourcePath` if omitted).

```csv
SourcePath,DestPath
folder1\file1.txt,folder1\file1.txt
folder2\report.doc,folder2\report.doc
subfolder\data,subfolder\data
```

Paths are **relative to the share root** — do not include `\\server\share\` prefixes or drive letters.
...
| `stamp-metadata-YYYYMMDD-HHMMSS-failures.csv` | CSV of all failed items (SourcePath, DestPath, Error). Can be passed directly to `-InputFile` for retry. |
