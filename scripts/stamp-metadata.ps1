...
.PARAMETER InputFile
    CSV file with columns: SourcePath, DestPath (paths relative to share root).
...
try {
    ...
    if ($InputFile) {
        ...
        if ($columns -notcontains "SourcePath") {
            Write-Log "InputFile must have a SourcePath column. Found: $($columns -join ', ')" "ERROR"; exit 1
        }
        ...
        foreach ($row in $csv) {
            ...
            if ([string]::IsNullOrWhiteSpace($row.SourcePath)) {
                Write-Log "Skipping empty SourcePath at CSV line $lineNum" "WARN"; continue
            }
            $destRel = if ($columns -contains "DestPath" -and -not [string]::IsNullOrWhiteSpace($row.DestPath)) {
                $row.DestPath
            } else { $row.SourcePath }
            $filePairs += @{
                Src     = Join-Path $srcDrive $row.SourcePath
                Dest    = Join-Path $dstDrive $destRel
                CsvLine = $lineNum
            }
        }
        ...
        foreach ($item in $allItems) {
            ...
            $filePairs += @{ Src = $item.FullName; Dest = Join-Path $dstDrive $rel; CsvLine = 0 }
        }
