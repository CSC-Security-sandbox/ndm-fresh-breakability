...
| `wal_compression` | off | **zstd** | 2–4× less WAL I/O; PG 17 native zstd — small CPU cost, big disk savings |
...
| Write throughput (TPS) | ~**2× higher** (from `max_wal_size` 1 GB → 4 GB, `wal_buffers` 8 MB → 32 MB, `wal_compression = zstd`) |
