...
export class MigrationConflictService {
  ...
  async checkMigrationConflicts(
    data: MigrationConflictCheckData,
  ): Promise<PreCheckCircularDependency[]> {
    try {
      const migrationConflicts: PreCheckCircularDependency[] = [];
      this.logger.log(
        `Checking migration conflicts for data: ${JSON.stringify(data)}`,
      );
      ...
      this.logger.log(
        `Migration conflict check completed. Found ${migrationConflicts.length} conflicts`,
      );
      return migrationConflicts;
    } catch (error) {
   ...
   * @returns Array of migration conflicts found
   */
  async verifyMigrationConflicts(
    data: MigrationConflictCheckData,
  ): Promise<PreCheckCircularDependency[]> {
    return this.checkMigrationConflicts(data);
  }
   ...
   * @returns Array of circular dependencies found
   */
  async verifyCircularTaskDependency(
    data: MigrationConflictCheckData,
  ): Promise<PreCheckCircularDependency[]> {
    return this.checkMigrationConflicts(data);
  }
   ...
   */
  private async findMigrationConflictForConfig(
    config: MigrationConflictCheckConfig,
  ): Promise<PreCheckCircularDependency[]> {
    ...
    const uniqueDestinationPathConflicts =
      destinationPathConflictingJobs.filter(
    ...
    for (const job of uniqueDestinationPathConflicts) {
      ...
      if (
        ...
        const isExactDuplicate =
          job.sourcePathId === config.sourcePathId &&
          this.isSameDirectory(
    ...
    for (const job of uniqueSourcePathConflicts) {
      ...
      if (
        ...
        const isExactDuplicate = this.isSameDirectory(
          config.sourceDirectoryPath,
          job.sourceDirectoryPath,
        );
   ...
   * @returns Array of migration conflicts found
   */
  async getMigrationConflictDetails(
    sourcePathId: string,
   ...
   * @returns Array of circular dependencies found
   */
  async getCircularDependencyDetails(
    sourcePathId: string,
    destinationPathIds: string[],
  ): Promise<PreCheckCircularDependency[]> {
