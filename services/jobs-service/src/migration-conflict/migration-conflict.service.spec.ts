...
describe('MigrationConflictService', () => {
  ...
  describe('checkMigrationConflicts', () => {
    ...
    it('returns one conflict with type circular when an active job has source=our destination and target=our source', async () => {
      ...
      mockJobConfigRepository.find
        .mockResolvedValueOnce([mockConflictingJob])
    ...
    it('throws when repository find fails', async () => {
      ...
      mockJobConfigRepository.find.mockRejectedValue(
      ...
      try {
        ...
        expect(error.message).toBe(
          'Failed to check migration conflicts: Error: Database error',
        );
      }
    });
  });
  ...
  describe('verifyMigrationConflicts', () => {
    it('delegates to checkMigrationConflicts and returns its result', async () => {
      ...
      const spy = jest
        .spyOn(service, 'checkMigrationConflicts')
        .mockResolvedValue([]);
  ...
  describe('checkCircularDependency', () => {
    it('should delegate to checkMigrationConflicts and return same result', async () => {
      const data: MigrationConflictCheckData = {
        migrateConfigs: [
          { sourcePathId: 'src-1', destinationPathId: ['dest-1'] },
        ],
      };
      mockJobConfigRepository.find.mockResolvedValue([]);
  ...
  describe('hasCircularDependencies', () => {
    it('should return true when conflicts exist', async () => {
      jest
        .spyOn(service, 'checkMigrationConflicts')
        .mockResolvedValue([{ jobId: 'j1', conflictType: 'circular' } as any]);
  ...
  describe('getCircularDependencyDetails', () => {
    it('should call checkMigrationConflicts with formatted migrateConfigs', async () => {
      const spy = jest
        .spyOn(service, 'checkMigrationConflicts')
        .mockResolvedValue([]);
      await service.getCircularDependencyDetails('source-1', [
        'dest-1',
        'dest-2',
      ]);
  ...
  describe('hasMigrationConflicts', () => {
    it('returns true when checkMigrationConflicts returns at least one conflict', async () => {
      ...
      jest
        .spyOn(service, 'checkMigrationConflicts')
        .mockResolvedValue([mockDependency]);
  ...
  describe('Edge cases and error scenarios', () => {
    ...
    it('returns multiple conflicts when several jobs create circular dependency for one config', async () => {
      ...
      mockJobConfigRepository.find
        .mockResolvedValueOnce(mockConflictingJobs)
        .mockResolvedValueOnce([])
        .mockResolvedValueOnce([]);
    ...
    it('returns both circular and destination conflicts when different jobs cause each', async () => {
      ...
      mockJobConfigRepository.find
        .mockResolvedValueOnce([mockTraditionalConflictingJob])
    ...
    it('returns circular conflict when the conflicting job is active', async () => {
      ...
      mockJobConfigRepository.find
        .mockResolvedValueOnce([mockActiveCircularJob])
        .mockResolvedValueOnce([])
        .mockResolvedValueOnce([]);
  ...
  describe('Directory overlap and exact duplicate', () => {
    ...
    it('should not report destination conflict when destination dir and job target dir do not overlap', async () => {
      ...
      mockJobConfigRepository.find
        .mockResolvedValueOnce([])
        .mockResolvedValueOnce([mockDestJob])
        .mockResolvedValueOnce([]);
      const result = await service.checkMigrationConflicts(data);
      expect(result).toHaveLength(0);
    });
    ...
    it('should not report destination conflict when exact duplicate (same source path, same source dir, same dest dir)', async () => {
      ...
      mockJobConfigRepository.find
        .mockResolvedValueOnce([])
        .mockResolvedValueOnce([mockExactDuplicateJob])
        .mockResolvedValueOnce([]);
      const result = await service.checkMigrationConflicts(data);
      expect(result).toHaveLength(0);
    });
    ...
    it('should report destination conflict when directories overlap but not exact duplicate', async () => {
      ...
      mockJobConfigRepository.find
        .mockResolvedValueOnce([])
        .mockResolvedValueOnce([mockOverlapJob])
        .mockResolvedValueOnce([]);
      const result = await service.checkMigrationConflicts(data);
      expect(result).toHaveLength(1);
    ...
    it('should treat export-level (null/empty) directory as overlapping with any dir', async () => {
      ...
      mockJobConfigRepository.find
        .mockResolvedValueOnce([mockJobWithDirs])
        .mockResolvedValueOnce([])
        .mockResolvedValueOnce([]);
      const result = await service.checkMigrationConflicts(data);
      expect(result).toHaveLength(1);
    ...
    it('should not double-count job that is both circular and destination conflict', async () => {
      ...
      mockJobConfigRepository.find
        .mockResolvedValueOnce([mockSameJob])
        .mockResolvedValueOnce([mockSameJob])
        .mockResolvedValueOnce([]);
      const result = await service.checkMigrationConflicts(data);
      expect(result).toHaveLength(1);
  ...
  describe('Directory-level conflict detection methods', () => {
    ...
    describe('hasDirectoryOverlap', () => {
      it('should return true when either directory is null/undefined (export-level job)', () => {
        expect((service as any).hasDirectoryOverlap(null, '/some/path')).toBe(
          true,
        );
        expect((service as any).hasDirectoryOverlap('/some/path', null)).toBe(
          true,
        );
        expect(
          (service as any).hasDirectoryOverlap(undefined, '/some/path'),
        ).toBe(true);
        expect(
          (service as any).hasDirectoryOverlap('/some/path', undefined),
        ).toBe(true);
        expect((service as any).hasDirectoryOverlap(null, null)).toBe(true);
        expect((service as any).hasDirectoryOverlap(undefined, undefined)).toBe(
          true,
        );
      });
      ...
      it('should return true when directories are exactly the same', () => {
        expect(
          (service as any).hasDirectoryOverlap('/path/to/dir', '/path/to/dir'),
        ).toBe(true);
      });
      ...
      it('should return true when directories are the same after normalization', () => {
        ...
        expect(
          (service as any).hasDirectoryOverlap(
            '/path/to/dir///',
            '/path/to/dir',
          ),
        ).toBe(true);
      ...
      it('should return true when one directory is parent of another', () => {
        expect(
          (service as any).hasDirectoryOverlap('/parent', '/parent/child'),
        ).toBe(true);
        expect(
          (service as any).hasDirectoryOverlap('/parent/child', '/parent'),
        ).toBe(true);
        expect(
          (service as any).hasDirectoryOverlap(
            '/grandparent/parent',
      ...
      it('should return false when directories are siblings', () => {
        ...
        expect((service as any).hasDirectoryOverlap('/dir1', '/dir2')).toBe(
          false,
        );
      });
      ...
      it('should return false when directories have similar names but are not related', () => {
        expect(
          (service as any).hasDirectoryOverlap('/parent', '/parent-similar'),
        ).toBe(false);
        expect(
          (service as any).hasDirectoryOverlap('/parent-similar', '/parent'),
        ).toBe(false);
        expect((service as any).hasDirectoryOverlap('/data', '/database')).toBe(
          false,
        );
      });
      ...
      it('should handle complex nested paths correctly', () => {
        expect(
          (service as any).hasDirectoryOverlap('/a/b/c', '/a/b/c/d/e/f'),
        ).toBe(true);
        expect(
          (service as any).hasDirectoryOverlap('/a/b/c/d/e/f', '/a/b/c'),
        ).toBe(true);
        expect((service as any).hasDirectoryOverlap('/a/b/c', '/a/b/d')).toBe(
          false,
        );
      });
    });
    ...
    describe('isSameDirectory', () => {
      it('should return true for identical paths', () => {
        expect(
          (service as any).isSameDirectory('/path/to/dir', '/path/to/dir'),
        ).toBe(true);
      });
      ...
      it('should return true when both are null/undefined', () => {
        ...
        expect((service as any).isSameDirectory(undefined, undefined)).toBe(
          true,
        );
      ...
      it('should return false for different paths', () => {
        expect(
          (service as any).isSameDirectory('/path/to/dir1', '/path/to/dir2'),
        ).toBe(false);
        expect(
          (service as any).isSameDirectory('/parent/child', '/parent'),
        ).toBe(false);
        expect(
          (service as any).isSameDirectory('/parent', '/parent/child'),
        ).toBe(false);
      });

      it('should return false when one is null and other is a path', () => {
        expect((service as any).isSameDirectory(null, '/some/path')).toBe(
          false,
        );
        expect((service as any).isSameDirectory('/some/path', null)).toBe(
          false,
        );
        expect((service as any).isSameDirectory(undefined, '/some/path')).toBe(
          false,
        );
        expect((service as any).isSameDirectory('/some/path', undefined)).toBe(
          false,
        );
      });
      ...
      it('should handle edge cases with similar but different paths', () => {
        expect(
          (service as any).isSameDirectory('/parent', '/parent-similar'),
        ).toBe(false);
        expect((service as any).isSameDirectory('/data', '/database')).toBe(
          false,
        );
        expect(
          (service as any).isSameDirectory('/path/sub', '/path/sub-dir'),
        ).toBe(false);
      });
    });
  });
