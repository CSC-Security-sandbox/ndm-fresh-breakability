      ...
      generalErrorCodes.forEach(errorCode => {
        describe(`${errorCode}`, () => {
          ...
          it('should return remedy when remedy is provided and system message is empty/null/undefined', () => {
            const result1 = getErrorDisplayMessage(errorCode, '', mockRemedyDescription);
            const result2 = getErrorDisplayMessage(errorCode, null as any, mockRemedyDescription);
            const result3 = getErrorDisplayMessage(errorCode, undefined as any, mockRemedyDescription);
          ...
          it('should return system message when remedy is not provided/empty string', () => {
            const result1 = getErrorDisplayMessage(errorCode, mockSystemMessage);
            const result2 = getErrorDisplayMessage(errorCode, mockSystemMessage, '');
