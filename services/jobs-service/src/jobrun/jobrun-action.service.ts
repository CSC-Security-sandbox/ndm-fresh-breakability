...
export class JobRunActionService {
  ...
  private async getPausedJobRuns(
    jobRunIds: string[],
  ): Promise<{ paused: JobRunEntity[]; invalid: string[] }> {
  ...
  async actions(jobRunActions: JobRunActionsReq) {
    switch (jobRunActions.action) {
      ...
      case JobRunActions.RESUME: {
        ...
        const { paused, invalid } = await this.getPausedJobRuns(
          jobRunActions.jobRuns,
        );
        if (paused.length === 0) {
          throw new BadRequestException('No paused job runs found to resume.');
        }
        const resumeResults = await this.resumeJobRuns(
          paused.map((jr) => jr.id),
        );
        if (invalid.length > 0) {
          return {
