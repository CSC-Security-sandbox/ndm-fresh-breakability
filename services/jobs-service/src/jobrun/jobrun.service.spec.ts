...
describe("JobRunService", () => {
  ...
  describe("addHocRun", () => {
    ...
    it("should throw BadRequestException with grouped conflict types for multiple migration conflicts", async () => {
      const mockJobConfigId = "job-config-id";
      const mockJobConfig = {
        id: mockJobConfigId,
        scheduler: ScheduleStatus.READY_TO_BE_SCHEDULED,
      ...
      jest.spyOn(jobConfigRepo, "findOne").mockResolvedValue(mockJobConfig as any);
      const conflictServiceMock = module.get(MigrationConflictService) as any;
      conflictServiceMock.checkMigrationConflicts.mockResolvedValue(mockConflicts);

      await expect(service.addHocRun(mockJobConfigId)).rejects.toThrow(
        new BadRequestException(
          `Migration conflicts detected (circular, path_overlap) for job config ${mockJobConfigId}`
        )
      );
    });

    it("should create retry run when jobRunId is provided and validation passes", async () => {
      const mockJobConfigId = "job-config-id";
      ...
      jest.spyOn(jobConfigRepo, "findOne").mockResolvedValue(mockJobConfig as any);
      const conflictServiceMock = module.get(MigrationConflictService) as any;
      conflictServiceMock.checkMigrationConflicts.mockResolvedValue([]);

      jest.spyOn(jobRunRepo, "findOne").mockResolvedValue({ id: jobRunId } as any);
      const latestRunQb: any = {
      ...
      jest.spyOn(jobRunRepo, "createQueryBuilder").mockReturnValue(latestRunQb);

      const errorCountQb: any = {
        innerJoin: jest.fn().mockReturnThis(),
        where: jest.fn().mockReturnThis(),
        andWhere: jest.fn().mockReturnThis(),
      ...
      jest
        ...
        .mockReturnValue(errorCountQb);

      jest
        .spyOn(jobRunInitService, "createJobRun")
        .mockResolvedValue("retry run created" as any);

      const result = await service.addHocRun(mockJobConfigId, projectId, jobRunId);

      expect(result).toBe("retry run created");
      expect(jobRunRepo.findOne).toHaveBeenCalledWith({
        where: { id: jobRunId, jobConfigId: mockJobConfigId },
      });
      expect(jobRunInitService.createJobRun).toHaveBeenCalledWith(
        mockJobConfigId,
        expect.any(Date),
    ...
    it("should throw NotFoundException when retry job run does not exist for job config", async () => {
      const mockJobConfigId = "job-config-id";
      const jobRunId = "run-404";
      const mockJobConfig = {
        id: mockJobConfigId,
        scheduler: ScheduleStatus.READY_TO_BE_SCHEDULED,
        status: JobStatus.Active,
      ...
      await expect(service.addHocRun(mockJobConfigId, undefined, jobRunId)).rejects.toThrow(
        NotFoundException
      );
    });

    it("should throw BadRequestException when provided retry job run is not latest", async () => {
      const mockJobConfigId = "job-config-id";
      const jobRunId = "run-old";
      const mockJobConfig = {
        id: mockJobConfigId,
        scheduler: ScheduleStatus.READY_TO_BE_SCHEDULED,
        status: JobStatus.Active,
    ...
    it("should throw BadRequestException when retry run job type is not supported", async () => {
      const mockJobConfigId = "job-config-id";
      const jobRunId = "run-1";
      const mockJobConfig = {
        id: mockJobConfigId,
    ...
    it("should throw BadRequestException when there are no failed operations for retry", async () => {
      const mockJobConfigId = "job-config-id";
      const jobRunId = "run-1";
      const mockJobConfig = {
        id: mockJobConfigId,
  ...
  it("should handle completed job runs with jobstats", async () => {
    ...
    it("should use MV stats for scannedFilesCount (jobstats is ignored)", async () => {
    const filter = { projectId: "project123" };
    const mockJobRuns = [
      {
        jobrunid: "run-str",
    ...
    jest.spyOn(service as any, "fetchBatchMvStats").mockResolvedValue({
      "run-str": { fileCount: "42", directoryCount: "4", totalSize: "2048" },
    });
    jest.spyOn(service as any, "fetchBatchErrorCounts").mockResolvedValue({});
  ...
  it("should fall back to MV stats when jobstats is a malformed JSON string", async () => {
    ...
    it("should use MV stats regardless of jobstats object on the row", async () => {
    const filter = { projectId: "project123" };
    const mockJobRuns = [
      {
        jobrunid: "run-obj",
    ...
    jest.spyOn(service as any, "fetchBatchMvStats").mockResolvedValue({
      "run-obj": { fileCount: "99", directoryCount: "9", totalSize: "8192" },
    });
    jest.spyOn(service as any, "fetchBatchErrorCounts").mockResolvedValue({});
  ...
  describe("updateJobRunStatus", () => {
    it("should persist fresh terminal counters in jobStats for completed runs", async () => {
      const jobRunId = "job-1";
      const status = JobRunStatus.Completed;
      const jobRunDetails = { id: jobRunId, jobConfigId: "cfg-1" };
      ...
      jest.spyOn(jobConfigRepo, "update").mockResolvedValue(undefined as any);
      jest.spyOn(errorRemedyService, "getDistinctErrorCodes").mockResolvedValue([] as any);
      jest.spyOn(service, "calculateJobRunStats").mockResolvedValue({
        fileCount: "4",
        directories: "1",
      ...
      jest.spyOn(service as any, "calculateInventoryStatsFromBaseTable").mockResolvedValue({
        fileCount: "4",
        directories: "1",
        totalSize: "2048",
        deletedCount: "3",
      ...
      expect(updateSpy).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          status: JobRunStatus.Completed,
          endTime: expect.any(Date),
          jobStats: expect.objectContaining({
            newlyCopiedCount: "7",
            modifiedCount: "8",
            deletedCount: "3",
            excludedCount: "5",
    ...
    it("should persist MV stats into job_stats when terminal (no Redis read)", async () => {
      ...
      jest.spyOn(service as any, "calculateInventoryStatsFromBaseTable").mockResolvedValue({ fileCount: "10", directories: "2", totalSize: "4096", deletedCount: "0", excludedCount: "0", newlyCopiedCount: "0", modifiedCount: "0", migratedSize: "0" });
      jest.spyOn(errorRemedyService, "getDistinctErrorCodes").mockResolvedValue([] as any);
      (redisService.getLiveStats as jest.Mock).mockResolvedValueOnce({ fileCount: "0", dirCount: "0", totalSize: "0", newlyCopiedCount: "0", recopiedCount: "0", deletedCount: "0" });
    ...
    it("should persist MV stats into job_stats when transitioning to Paused (no Redis read)", async () => {
      ...
      jest.spyOn(service as any, "calculateInventoryStatsFromBaseTable").mockResolvedValue({ fileCount: "200", directories: "10", totalSize: "8192", deletedCount: "0", excludedCount: "0", newlyCopiedCount: "0", modifiedCount: "0", migratedSize: "0" });
      jest.spyOn(errorRemedyService, "getDistinctErrorCodes").mockResolvedValue([] as any);
      (redisService.getLiveStats as jest.Mock).mockResolvedValueOnce({ fileCount: "0", dirCount: "0", totalSize: "0", newlyCopiedCount: "0", recopiedCount: "0", deletedCount: "0" });
  ...
  describe('JobRunService - additional methods', () => {
    describe('calculateJobRunStats', () => {
      it('should calculate and return job run stats from MV', async () => {
      ...
      it('should default to "0" when inventory summary values are undefined', async () => {
        ...
        jest.spyOn(jobRunRepo, 'findOne').mockResolvedValueOnce({ id: jobRunId, jobStats: null, endTime: null } as any);
        jest.spyOn(jobStatsSummaryMvRepo, 'findOne').mockResolvedValueOnce({ deletedCount: 0, excludedCount: 0, newlyCopiedCount: 0, recopiedCount: 0 } as any);
        jest.spyOn(service, 'getErrorCounts').mockResolvedValueOnce(mockErrorCounts);
      ...
      it('should return MV data even when job_stats snapshot exists on the job run', async () => {
        const jobRunId = 'jobRunId';
        const mockMvStats = {
          fileCount: '100',
          directoryCount: '10',
      ...
      it('should return MV data when fileCount is "0" but directoryCount is non-zero', async () => {
      ...
      it('should return MV data when only totalSize is non-zero', async () => {
  ...
  /**
   * getJobRunLiveStats:
   * - Terminal / Paused: always reads from MV (jobStatsSummaryMvRepo), never from job_stats or Redis.
   * - Running / Pausing / Stopping: reads from Redis liveStats; modifiedCount maps from recopiedCount.
   */
  describe("getJobRunLiveStats", () => {
    it("should return MV stats when job is in terminal status (Completed)", async () => {
      const jobRunId = "run1";
      const mockJobRun = { id: jobRunId, status: JobRunStatus.Completed };
      const mockMv = { fileCount: "50", directoryCount: "5", totalSize: "1024", newlyCopiedCount: "0", recopiedCount: "0", deletedCount: "0", lastRefreshed: null };

      jest.spyOn(jobRunRepo, "findOne").mockResolvedValueOnce(mockJobRun as any);
      jest.spyOn(jobRunRepo, "findOne").mockResolvedValueOnce(mockJobRun as any);
      jest.spyOn(jobStatsSummaryMvRepo, "findOne").mockResolvedValueOnce(mockMv as any);
      jest.spyOn(service, "getErrorCounts").mockResolvedValueOnce([]);
    ...
    it("should return MV stats when job is Paused (not reading Redis)", async () => {
      const jobRunId = "run-paused";
      const mockJobRun = { id: jobRunId, status: JobRunStatus.Paused };
      const mockMv = { fileCount: "200", directoryCount: "10", totalSize: "8192", newlyCopiedCount: "0", recopiedCount: "0", deletedCount: "0", lastRefreshed: null };
    ...
    it("should return MV extended fields (newlyCopiedCount, modifiedCount, deletedCount) for terminal jobs", async () => {
      const jobRunId = "run-snap-extended";
      const mockJobRun = { id: jobRunId, status: JobRunStatus.Completed };
      const mockMv = {
        fileCount: "1",
        directoryCount: "0",
    ...
    it("should return MV stats for a Paused job (even when MV has zero counts)", async () => {
      const jobRunId = "run-paused-no-snap";
      const mockJobRun = { id: jobRunId, status: JobRunStatus.Paused };
      const mockMv = { fileCount: "150", directoryCount: "8", totalSize: "4096", newlyCopiedCount: "0", recopiedCount: "0", deletedCount: "0", lastRefreshed: null };
    ...
    it("should return MV dirCount when fileCount is 0 but directoryCount is non-zero", async () => {
      ...
      const mockJobRun = { id: jobRunId, status: JobRunStatus.Completed };
      const mockMv = { fileCount: "0", directoryCount: "3", totalSize: "0", newlyCopiedCount: "0", recopiedCount: "0", deletedCount: "0", lastRefreshed: null };
    ...
    it("should return MV totalSizeBytes when only totalSize is non-zero", async () => {
      const jobRunId = "run-size-only";
      const mockJobRun = { id: jobRunId, status: JobRunStatus.Completed };
      const mockMv = { fileCount: "0", directoryCount: "0", totalSize: "4096", newlyCopiedCount: "0", recopiedCount: "0", deletedCount: "0", lastRefreshed: null };
    ...
    it("should return MV stats for terminal job (Failed) — always reads MV not Redis", async () => {
      const jobRunId = "run-no-snapshot";
      const mockJobRun = { id: jobRunId, status: JobRunStatus.Failed };
      const mockMv = { fileCount: "20", directoryCount: "2", totalSize: "256", newlyCopiedCount: "0", recopiedCount: "0", deletedCount: "0", lastRefreshed: null };
  ...
  describe("additional branch coverage - pagination and excluded/skipped flows", () => {
    it("should return paginated failed operations with nextCursor when hasMore is true", async () => {
      const qb = {
        innerJoin: jest.fn().mockReturnThis(),
        select: jest.fn().mockReturnThis(),
        addSelect: jest.fn().mockReturnThis(),
        ...
        getRawMany: jest.fn().mockResolvedValue([
          { operationId: "op-1", filePath: "/a" },
          { operationId: "op-2", filePath: "/b" },
          { operationId: "op-3", filePath: "/c" },
        ]),
      };
      jest.spyOn(operationErrorRepo, "createQueryBuilder").mockReturnValue(qb as any);

      const result = await service.getFailedOperations("run-1", null, 2);
      ...
      expect(result).toEqual({
        ...
        nextCursor: "/b|undefined",
      });
      expect(qb.take).toHaveBeenCalledWith(3);
    });

    it("should apply cursor filter and return null nextCursor when no extra rows", async () => {
      const qb = {
        innerJoin: jest.fn().mockReturnThis(),
        select: jest.fn().mockReturnThis(),
        addSelect: jest.fn().mockReturnThis(),
        ...
        take: jest.fn().mockReturnThis(),
        getRawMany: jest.fn().mockResolvedValue([
          { operationId: "op-22", filePath: "/z", operationErrorId: "22" },
        ]),
      };
      jest.spyOn(operationErrorRepo, "createQueryBuilder").mockReturnValue(qb as any);

      const result = await service.getFailedOperations("run-1", "/x|10", 5);

      expect(result).toEqual({
        data: [{ id: "op-22", fPath: "/z" }],
        nextCursor: null,
      });
      expect(qb.andWhere).toHaveBeenCalledWith(
        "(o.fPath > :cursorPath OR (o.fPath = :cursorPath AND oe.id > :cursorId))",
        { cursorPath: "/x", cursorId: "10" }
      );
    });

    it("should throw NotFoundException in addExcludedSkippedEntries when jobRun is missing", async () => {
      jest.spyOn(jobRunRepo, "findOne").mockResolvedValueOnce(null as any);

      await expect(
        service.addExcludedSkippedEntries("missing", [{ path: "/a" }], [{ path: "/b" }])
      ).rejects.toThrow(NotFoundException);
    });

    it("should return added 0 when excluded and skipped are empty", async () => {
      jest.spyOn(jobRunRepo, "findOne").mockResolvedValueOnce({ id: "run-2" } as any);
      const querySpy = jest.spyOn(dataSource, "query");

      const result = await service.addExcludedSkippedEntries("run-2", [], []);
    ...
    it("should upsert rows and prefer skipped for duplicates", async () => {
      ...
      const result = await service.addExcludedSkippedEntries(
        "run-3",
        [{ path: "/same", isDirectory: false }, { path: "C:\\temp\\x", isDirectory: false }],
        [{ path: "/same", isDirectory: false }, { path: "/dir", isDirectory: true }]
      );
    ...
    it("should rethrow error from getWorkerSetupErrors query builder", async () => {
      const qb = {
        where: jest.fn().mockReturnThis(),
        andWhere: jest.fn().mockReturnThis(),
        getMany: jest.fn().mockRejectedValue(new Error("db-fail")),
      };
      jest.spyOn(workerJobRunMapRepo, "createQueryBuilder").mockReturnValue(qb as any);

      await expect(service.getWorkerSetupErrors("run-err")).rejects.toThrow("db-fail");
    });

    it("should throw internal server error when getJobRunIdentityMappings has unexpected error", async () => {
      jest.spyOn(jobRunRepo, "findOne").mockRejectedValueOnce(new Error("db down"));

      await expect(service.getJobRunIdentityMappings("run-7")).rejects.toThrow(
        "Failed to fetch identity mappings for job run run-7"
      );
    });
  });
});
