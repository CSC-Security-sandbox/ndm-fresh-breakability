...
import { ErrorType, formatBytes } from '@netapp-cloud-datamigrate/jobs-lib';
...
describe('JobRunService', () => {
  ...
  beforeEach(async () => {
    ...
    jobConfigInventoryStatsRepo = module.get<
      Repository<JobConfigInventoryStatsEntity>
  ...
  describe('addHocRun', () => {
    ...
    it('should throw BadRequestException when circular dependency is detected for MIGRATE job', async () => {
      ...
      const circularDependencyMock = module.get(MigrationConflictService);
      (
        circularDependencyMock.checkMigrationConflicts as jest.Mock
      ).mockResolvedValue(mockCircularDependency);
      ...
      try {
        ...
        expect(error.message).toBe(
          `Circular conflict detected for job config ${mockJobConfigId}`,
        );
        expect(error.options.cause).toEqual(mockCircularDependency);
      }

      expect(
        circularDependencyMock.checkMigrationConflicts,
  ...
  it('should use MV stats for completed job runs without persisted jobstats', async () => {
    const filter = { projectId: 'project123' };
    const mockJobRuns = [
      {
        jobrunid: 'run2',
    ...
    const calculateJobRunStatsSpy = jest
      .spyOn(service, 'calculateJobRunStats')
      .mockResolvedValue(mockMvStats);

    const result = await service.getJobAllRuns(filter);

    expect(calculateJobRunStatsSpy).toHaveBeenCalledWith('run2');
  ...
  describe('getErrorCounts', () => {
    ...
    it('should count correct setupFailedErrors for setupFailed error type when other already there', async () => {
      const mockJobRunId = 'jobRunId';
      const mockErrorCounts = [{ errortype: 'FATAL_ERROR', count: 1 }];
    ...
    describe('Comprehensive Error Count Scenarios with Query Testing', () => {
      ...
      it('should count errors from both Source and Destination separately', async () => {
        ...
        const mockErrorCounts = [{ errortype: 'FATAL_ERROR', count: 2 }];
      ...
      it('should count mixed FATAL_ERROR and TRANSIENT_ERROR separately', async () => {
        ...
        expect(result.find((e) => e.errortype === 'FATAL_ERROR')?.count).toBe(
          5,
        );
        expect(
          result.find((e) => e.errortype === 'TRANSIENT_ERROR')?.count,
        ).toBe(3);
      });
      ...
      it('should handle job run with only FATAL_ERROR type', async () => {
        ...
        const mockErrorCounts = [{ errortype: 'FATAL_ERROR', count: 12 }];
      ...
      it('should handle job run with only TRANSIENT_ERROR type', async () => {
        ...
        const mockErrorCounts = [{ errortype: 'TRANSIENT_ERROR', count: 8 }];
      ...
      it('should exclude RECOVERABLE_ERROR from count', async () => {
        ...
        expect(result).toEqual(mockErrorCounts);
        expect(
          result.find((e) => e.errortype === 'RECOVERABLE_ERROR'),
        ).toBeUndefined();
        expect(result.find((e) => e.errortype === 'FATAL_ERROR')?.count).toBe(
          3,
        ); // 6 total - 3 recoverable = 3
      });

      it('should add worker setup errors to FATAL_ERROR count', async () => {
        const mockJobRunId = 'job-run-with-setup-errors';
        const mockErrorCounts = [{ errortype: 'FATAL_ERROR', count: 3 }];
  ...
  describe('getJobRunErrors', () => {
    ...
    beforeEach(() => {
      repoQuerySpy = jest.spyOn(operationErrorRepo, 'query').mockResolvedValue([
        {
          id: 'errorId1',
          errorMessage: 'Err 1',
    ...
    it('should return grouped rows + total', async () => {
      ...
      jest
        .spyOn(service as any, 'getWorkerSetupErrors')
        .mockResolvedValueOnce(setupFailedErrors);
      jest.spyOn(errorRemedyService, 'findByErrorCodes').mockReturnValue([
        {
          errorCode: 'E1',
          description: 'desc',
  ...
  describe('checkWorkerHealth', () => {
    ...
    it('should log error if anything goes wrong', async () => {
      ...
      try {
        ...
        expect(loggerSpy).toHaveBeenCalledWith(
          'Error occurred while checking worker health:',
          error,
        );
      }
    });
  ...
  describe('updateWorkerResponse', () => {
    it('should update worker response', async () => {
      ...
      const response = {
        workerResponse: { code: 'TEST', operation: 'test_operation' },
      };
    ...
    it('should throw error if update got failed', async () => {
      ...
      await expect(
        service.updateWorkerResponse(workerId, jobRunId, response),
  ...
  describe('JobRunService - additional methods', () => {
    describe('calculateJobRunStats', () => {
      it('should calculate and return job run stats', async () => {
        ...
        jest
          .spyOn(jobRunRepo, 'findOne')
          .mockResolvedValueOnce(mockJobRun as any);
        jest
          .spyOn(jobStatsSummaryMvRepo, 'findOne')
          .mockResolvedValueOnce(mockJobStatsSummary as any);
        jest
          .spyOn(service, 'getErrorCounts')
          .mockResolvedValueOnce(mockErrorCounts);
      ...
      it('should throw NotFoundException if job run not found', async () => {
        ...
        await expect(service.calculateJobRunStats('missingId')).rejects.toThrow(
          'Job Run with id missingId not found',
        );
      });
    });
    ...
    describe('getJobRunErrors', () => {
      it('should return error data and total for non-FATAL_ERROR', async () => {
        ...
        jest
          ...
          .mockReturnValue([{ errorCode: 'E1' }] as any);
      ...
      it('should append setupFailedErrors for FATAL_ERROR', async () => {
        ...
        const mockData = [
          { id: '1', errorType: 'FATAL_ERROR', occurrence: '1' },
        ];
        const mockTotal = { total: '1' };
        const setupFailedErrors = [
          {
    ...
    describe('updateWorkerResponse', () => {
      it('should update worker response', async () => {
        ...
        await expect(
          service.updateWorkerResponse('jobRunId', 'workerId', { foo: 'bar' }),
      ...
      it('should throw error if update fails', async () => {
        ...
        await expect(
          service.updateWorkerResponse('jobRunId', 'workerId', {}),
      ...
      it('should throw error if updateWorkerResponse fails', async () => {
        ...
        jest
          ...
          .mockRejectedValue(new Error('fail'));
        await expect(
          service.updateWorkerResponse(jobRunId, workerId, workerResponse),
        ).rejects.toThrow('Failed to update worker response: Error: fail');
      });
      ...
      it('should return worker setup errors', async () => {
        ...
        const mockErrors = [
          {
            workerResponse: { code: 'SETUP_WORKER_FAILURE', status: 'FAILED' },
          },
        ];
        jest
          .spyOn(workerJobRunMapRepo, 'find')
          .mockResolvedValue(mockErrors as any);
      ...
      it('should call workerJobRunMapRepo.update and return result in updateWorkerResponse', async () => {
        ...
        const result = await service.updateWorkerResponse(
          jobRunId,
          workerId,
          workerResponse,
        );
      ...
      describe('checkWorkerHealth', () => {
        ...
        beforeEach(() => {
          ...
          jobRunRepoUpdateSpy = jest
            .spyOn(jobRunRepo, 'update')
            .mockResolvedValue(undefined);
          workerServiceUpdateWorkerStatusSpy = jest.spyOn(
            workerService,
            'updateWorkerStatus',
          );
        ...
        it('should do nothing if no running job runs are found', async () => {
          ...
          expect(loggerLogSpy).toHaveBeenCalledWith(
            'Checking the health of workers',
          );
        ...
        it('should warn and continue if a jobRun has no workers', async () => {
          ...
          expect(loggerWarnSpy).toHaveBeenCalledWith(
            'No workers found for jobRunId: job1',
          );
          expect(jobRunRepoUpdateSpy).not.toHaveBeenCalled();
        });
    ...
    describe('updateWorkerResponse', () => {
      it('should update worker response successfully', async () => {
        ...
        const mockWorkerResponse = {
          status: 'SUCCESS',
          message: 'Worker setup completed',
        };
    ...
    describe('checkWorkerHealth', () => {
      it('should pause job run when all workers are offline', async () => {
        ...
        jest
          .spyOn(workerService, 'updateWorkerStatus')
          .mockReturnValue([{ status: WorkerStatus.Offline }] as any);
        jest
          .spyOn(jobRunRepo, 'update')
          .mockResolvedValue({ affected: 1 } as any);
    ...
    describe('updateWorkerResponse', () => {
      ...
      it('should throw error when update fails', async () => {
        ...
        await expect(
          service.updateWorkerResponse(
            mockJobRunId,
            mockWorkerId,
            mockWorkerResponse,
          ),
        ).rejects.toThrow(
          'Failed to update worker response: Error: Database error',
        );
      });
    });
    ...
    describe('sendErrorRemedyEmail', () => {
      ...
      it('should not send email when no error codes provided', async () => {
        ...
        const errorRemedySpy = jest.spyOn(
          errorRemedyService,
          'findByErrorCodes',
        );
    ...
    describe('getJobRunIdentityMappings', () => {
      ...
      it('should throw NotFoundException when job run not found', async () => {
        ...
        jest.spyOn(jobRunRepo, 'findOne').mockResolvedValue(null);
        await expect(
          service.getJobRunIdentityMappings(mockJobRunId),
        ).rejects.toThrow(
  ...
  describe('recordAsupStatsForJobRun (via updateJobRunStatus)', () => {
    ...
    const mockPassedStats = { fileCount: 250, dirCount: 5, totalSize: '50000' };
    ...
    beforeEach(() => {
      ...
      jest.spyOn(jobRunRepo, 'update').mockResolvedValue(undefined);
      jest.spyOn(jobConfigRepo, 'update').mockResolvedValue(undefined);
      // getErrorCounts is called when passedStats are provided
    ...
    it('should insert ASUP stats for a completed discover job run', async () => {
      ...
      jest.spyOn(jobConfigRepo, 'findOne').mockResolvedValue(makeJobConfig());
      ...
      await service.updateJobRunStatus(
        jobRunId,
      ...
      expect(dataSource.transaction).toHaveBeenCalled();
      const transactionCb = (dataSource.transaction as jest.Mock).mock
        .calls[0][0];
    ...
    it('should insert ASUP stats for a stopped migration job run', async () => {
      ...
      jest
        .spyOn(jobConfigRepo, 'findOne')
        .mockResolvedValue(makeJobConfig({ jobType: JobType.MIGRATE }));
        ...
        async (cb: any) => {
          const mockManager = {
            ...
            findOne: jest
              .fn()
              .mockResolvedValueOnce(
                makeJobConfig({ jobType: JobType.MIGRATE }),
              )
              .mockResolvedValueOnce({
                id: projectId,
    ...
    it('should not record ASUP stats for failed job runs', async () => {
      ...
      await transactionCb(mockManager);
      const asupCalls = mockManager.query.mock.calls.filter((call: any[]) =>
        (call[0] as string).includes('asup_stats'),
      );
          ...
          const mockManager = {
            ...
            findOne: jest
              ...
              .mockResolvedValueOnce(
                makeJobConfig({
                  sourcePath: {
                    fileServer: { protocol: Protocol.NFS, config: null },
                  },
                  targetPath: { fileServer: { config: null } },
                }),
              )
    ...
    it('should map cutover job type correctly', async () => {
      ...
      jest
        .spyOn(jobConfigRepo, 'findOne')
        .mockResolvedValue(makeJobConfig({ jobType: JobType.CUT_OVER }));
        ...
        async (cb: any) => {
          const mockManager = {
            ...
            findOne: jest
              .fn()
              .mockResolvedValueOnce(
                makeJobConfig({ jobType: JobType.CUT_OVER }),
              )
              .mockResolvedValueOnce({
                id: projectId,
