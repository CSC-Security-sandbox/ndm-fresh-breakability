...
describe('JobRunActionService', () => {
  ...
  beforeEach(() => {
    ...
    service = new JobRunActionService(
      jobRunRepo,
      workFlowService,
      loggerFactory,
    );
  ...
  describe('actions', () => {
    ...
    it('should resume job runs', async () => {
      const jobRuns = [
        { id: '3', workFlowId: 'wf3', status: JobRunStatus.Paused },
      ];
      jobRunRepo.find.mockResolvedValue(jobRuns);
      ...
      expect(jobRunRepo.find).toHaveBeenCalledWith({
        where: { id: expect.any(Object), status: JobRunStatus.Paused },
    ...
    it('should throw BadRequestException for invalid action', async () => {
      ...
      await expect(service.actions(req as any)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('resumeJobRuns', () => {
    ...
    it('should call logger.error when signal fails in resumeJobRuns', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(
        expect.stringMatching(/Failed to send signal to workflow/),
      );
    });
    ...
    it('should resume all job runs successfully', async () => {
      ...
      expect(jobRunRepo.update).toHaveBeenCalledWith(['1'], {
    ...
    it('should only resume paused jobs and return error for others', async () => {
      ...
      if (result[1].status === 'fulfilled') {
        expect(result[1].value.details).toMatch(
          /Operation Successful for jobRun: 2|Cannot resume job run/,
        );
      ...
      expect(
        result.every((r) => {
          if (r.status === 'fulfilled') {
            return r.value.details.match(
              /Cannot resume job run|Operation Successful for jobRun/,
            );
