...
export class JobRunController {
  ...
  @ApiOperation({
    summary:
      'Creates ad-hoc job run based on job config. Optionally retry failed items from a previous job run.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 400,
    description:
      'Job run is not in terminal state or no failed operations found (when retrying).',
  })
  @ApiBearerAuth()
  @Auth(Permission.ManageJob)
  @Post('/ad-hoc')
  async adhocRun(
    ...
    return this.jobRunService.addHocRun(
  ...
  async updateJobRunStatus(
    ...
    @Body()
    body?: {
      stats?: { fileCount?: number; dirCount?: number; totalSize?: string };
    },
  ) {
    this.logger.log(
      `Updating job run status: jobRunId=${jobRunId}, status=${status}, stats=${JSON.stringify(body?.stats)}`,
    );
    return await this.jobRunService.updateJobRunStatus(
      jobRunId,
      status,
      projectId,
      body?.stats,
  ...
  @ApiBody({
    description:
      'The response data returned by the worker (can include status, message, code, etc.)',
    type: Object,
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Returns identity mappings associated with the job run.',
  })
  @ApiResponse({ status: 404, description: 'Job run not found.' })
