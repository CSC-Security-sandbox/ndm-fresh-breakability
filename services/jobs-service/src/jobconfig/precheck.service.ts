...
export class PreCheckService {
  ...
  async initiatePreCheck(data: JobConfigPreCheck): Promise<any> {
    ...
    try {
      ...
      const preCheckPayload = this.createInitialPreCheckPayload(
        data.preserveAccessTime,
      ...
      const preCheckWorkPayload = this.prepareWorkflowPayload(
        preCheckPayload,
        traceId,
        data.options,
      );
  ...
  private createInitialPreCheckPayload(
  ...
  private async fetchVolumesWithWorkers(
    pathIds: string[],
  ...
  private async buildPreChecks(
    ...
    for (const config of data.migrateConfigs) {
      ...
      if (sourceVolume) {
        const discoveredSize = await this.getLatestDiscoveryInventorySize(
          config.sourcePathId,
        );
  ...
  async getLatestDiscoveryInventorySize(pathId: string): Promise<number> {
    ...
    const latestDiscoveryJobRun = await this.jobRunRepo
    ...
    const inventorySize = await this.inventoryRepo
      .createQueryBuilder('inventory')
      .where('inventory.job_run_id = :jobRunId', {
        jobRunId: latestDiscoveryJobRun.id,
      })
