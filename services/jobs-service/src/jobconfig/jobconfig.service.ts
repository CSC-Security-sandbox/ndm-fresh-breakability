...
import {JobConfigCutoverBulk, JobConfigDiscoverBulk, MigrateConfig,} from "./dto/jobdicoverybulk.dto";
...
export class JobConfigService {
    ...
    private async validatePathNames(migrateConfigs: MigrateConfig[]) {
        console.error("🚨 VALIDATION: Starting volume validation");
        console.error("🚨 VALIDATION: Number of configs =", migrateConfigs.length);
        ...
        console.error("🚨 VALIDATION: Path IDs to lookup =", pathIds);
        ...
        console.error("🚨 VALIDATION: Found volumes =", volumes.map(v => ({
            id: v.id,
            path: v.volumePath
        })));
        ...
        for (const config of migrateConfigs) {
            ...
            if (!volumeMap.has(config.sourcePathId)) {
                console.error("🚨 MISSING SOURCE VOLUME:", config.sourcePathId);
                throw new BadRequestException(
                    `Source volume not found: ${config.sourcePathId}`
                );
            }

            // Check destination volumes exist
            for (const destPathId of config.destinationPathId) {
                if (!volumeMap.has(destPathId)) {
                    console.error("🚨 MISSING DESTINATION VOLUME:", destPathId);
                    throw new BadRequestException(
                        `Destination volume not found: ${destPathId}`
                    );
                }
            }
        }

        console.error("🚨 VALIDATION: All volumes exist");
    }
    ...
    async createBulkMigrate(
        ...
        console.error("🚨 SERVICE: CSV data =", bulkMigrate.migrateConfigs);

        // 🔥 ADD DEBUG LOGS TO SEE CSV STRUCTURE
        console.error("🚨 DEBUG: migrateConfigs structure =", JSON.stringify(bulkMigrate.migrateConfigs.slice(0, 2), null, 2));
        console.error("🚨 DEBUG: Available CSV fields =", Object.keys(bulkMigrate.migrateConfigs[0] || {}));
        console.error("🚨 DEBUG: First config sourcePathId =", bulkMigrate.migrateConfigs[0]?.sourcePathId);

        // Enable validation to see what happens
        ...
        for (const config of bulkMigrate.migrateConfigs) {
            ...
            for (const destinationPath of config.destinationPathId) {
                // 🔥 ADD DEBUG: Look up actual paths before creating job
                const sourceVolume = await this.volumeRepo.findOne({
                    where: { id: config.sourcePathId },
                    select: { id: true, volumePath: true }
                });

                const destinationVolume = await this.volumeRepo.findOne({
                    where: { id: destinationPath },
                    select: { id: true, volumePath: true }
                });

                console.error("🚨 JOB CREATION DEBUG:");
                console.error("🚨   CSV Source ID:", config.sourcePathId);
                console.error("🚨   Actual Source Path:", sourceVolume?.volumePath || "NOT FOUND");
                console.error("🚨   CSV Destination ID:", destinationPath);
                console.error("🚨   Actual Destination Path:", destinationVolume?.volumePath || "NOT FOUND");
                ...
                if (
                    ...
                    for (const jobConfig of existingJobConfigs) {
                        if (jobConfig.status == JobStatus.InActive) {
                            ...
                            // 🔥 ADD ENHANCED DEBUG
                            console.error("🚨 INACTIVE JOB WARNING DEBUG:");
                            console.error("🚨   Job Config ID:", jobConfig.id);
                            console.error("🚨   Source UUID:", jobConfig.sourcePathId);
                            console.error("🚨   Source Path Lookup Result:", sourcePath);
                            console.error("🚨   Target UUID:", jobConfig.targetPathId);
                            console.error("🚨   Target Path Lookup Result:", targetPath);
                            console.error("🚨   Will use source path:", sourcePath?.volumePath || "NULL");
                            console.error("🚨   Will use target path:", targetPath?.volumePath || "NULL");
                ...
                if (existingSet.has(`${config?.sourcePathId}-${destinationPath}`)) {
                    ...
                    // 🔥 ADD DEBUG FOR JOB UPDATE
                    console.error("🚨 JOB UPDATE DEBUG:");
                    console.error("🚨   Updating existing job for Source UUID:", config?.sourcePathId);
                    console.error("🚨   Database source path:", sourceVolume?.volumePath || "NOT FOUND");
                    console.error("🚨   Updating existing job for Target UUID:", destinationPath);
                    console.error("🚨   Database target path:", destinationVolume?.volumePath || "NOT FOUND");
                    console.error("🚨   Existing job will continue using database paths, not CSV paths");
                    ...
                    // 🔥 ADD DEBUG AFTER UPDATE
                    console.error("🚨   Job updated successfully - workers will lookup paths by UUID");
                    console.error("🚨   Workers will use source path:", sourceVolume?.volumePath || "NULL");
                    console.error("🚨   Workers will use target path:", destinationVolume?.volumePath || "NULL");
                    console.error("🚨   =====================================");
                    ...
                    // 🔥 ADD DEBUG BEFORE CREATING NEW JOB
                    const sourceVolumeDebug = await this.volumeRepo.findOne({
                        where: { id: config?.sourcePathId },
                        select: { id: true, volumePath: true }
                    });

                    const targetVolumeDebug = await this.volumeRepo.findOne({
                        where: { id: destinationPath },
                        select: { id: true, volumePath: true }
                    });

                    console.error("🚨 NEW JOB CREATION DEBUG:");
                    console.error("🚨   Creating job with Source UUID:", config?.sourcePathId);
                    console.error("🚨   Database source path:", sourceVolumeDebug?.volumePath || "NOT FOUND");
                    console.error("🚨   Creating job with Target UUID:", destinationPath);
                    console.error("🚨   Database target path:", targetVolumeDebug?.volumePath || "NOT FOUND");
                    console.error("🚨   Job will use these UUIDs, but workers will lookup paths from database");
        ...
        if (jobConfigs.length > 0) {
            ...
            // 🔥 ADD DEBUG: Show final saved jobs with their path lookups
            console.error("🚨 SAVED JOBS DEBUG:");
            for (const savedJob of savedJobConfigs) {
                const finalSourcePath = await this.volumeRepo.findOne({
                    where: { id: savedJob.sourcePathId },
                    select: { volumePath: true }
                });

                const finalTargetPath = await this.volumeRepo.findOne({
                    where: { id: savedJob.targetPathId },
                    select: { volumePath: true }
                });

                console.error("🚨   Saved Job ID:", savedJob.id);
                console.error("🚨   Stored Source UUID:", savedJob.sourcePathId);
                console.error("🚨   Workers will use source path:", finalSourcePath?.volumePath || "NULL");
                console.error("🚨   Stored Target UUID:", savedJob.targetPathId);
                console.error("🚨   Workers will use target path:", finalTargetPath?.volumePath || "NULL");
                console.error("🚨   =====================================");
            }
