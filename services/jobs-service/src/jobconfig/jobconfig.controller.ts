...
export class JobConfigController {
  ...
  @ApiResponse({
    status: 201,
    description: 'Discovery job has been successfully created.',
  })
  @Auth(Permission.ManageJob)
  @ApiBearerAuth()
  @Post('/bulk-discovery')
  async createBulkDiscovery(
    @Body() bulkDiscovery: JobConfigDiscoverBulk,
  ): Promise<JobConfigEntity[]> {
    if (
      !bulkDiscovery.sourcePathIds ||
      bulkDiscovery.sourcePathIds.length === 0
    ) {
      throw new BadRequestException('Source path IDs cannot be empty.');
    }
    const jobConfig =
      await this.jobConfigService.createBulkDiscovery(bulkDiscovery);
    return jobConfig;
  }
  ...
  @ApiResponse({
    status: 201,
    description: 'Speed Test job has been successfully created.',
  })
  @Auth(Permission.ManageJob)
  @ApiBearerAuth()
  @Post('/speed-test')
  async createSpeedTest(
    @Body() speedTest: JobConfigSpeedTest,
  ...
  @ApiResponse({
    status: 200,
    description: 'Returns a list of all Speed jobs Runs.',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Speed test Result has been successfully Stored.',
  })
  @AuthWorker()
  @ApiBearerAuth()
  ...
  @ApiResponse({
    status: 201,
    description: 'Migrate job has been successfully created.',
  })
  @ApiResponse({
    status: 500,
  ...
  async createBulkMigrate(
    ...
    return await this.jobConfigService.createBulkMigrate(
      bulkMigrate,
      projectId,
    );
  }
  ...
  @ApiResponse({
    status: 201,
    description: 'Cutover job has been successfully created.',
  })
  @ApiResponse({
    status: 500,
    description: 'Internal Server Error - Unexpected error occurred.',
  ...
  async createBulkCutover(
    @Body() bulkCutover: JobConfigCutoverBulk,
  ...
  @Auth(Permission.ViewJob)
  @Get()
  async getAllJobConfig(
    @Query('projectId') projectId: string,
  ): Promise<JobListingDTO[]> {
    if (!projectId) {
      throw new BadRequestException(
        `Required parameters['ProjectId'] is missing in the request`,
      );
    }
  ...
  @ApiOperation({
    summary: 'Get jobfindallConfigPageDto: FindallConfigPageDto by ID',
  })
  @ApiResponse({ status: 200, description: 'Returns a job by its ID.' })
  @ApiResponse({ status: 404, description: 'Job not found.' })
  @Get(':id')
  ...
  @ApiResponse({
    status: 200,
    description: 'Successfully retrieved notice board details',
  })
  ...
  async getNoticeBoardDetailsByProjectId(
    @Param('projectId') projectId: string,
  ) {
    return await this.jobConfigService.getNoticeBoardDetailsByProjectId(
      projectId,
    );
  }
  ...
  @ApiResponse({
    status: 200,
    description: 'Discovery job configuration has been successfully updated.',
  })
  @ApiResponse({ status: 404, description: 'Job not found.' })
  @ApiResponse({
    status: 400,
    description:
      'Invalid job type - only discovery jobs can be updated with this endpoint.',
  })
  @ApiBearerAuth()
  @Auth(Permission.ManageJob)
  ...
  async updateDiscoveryJobConfig(
    ...
    if (jobEntity.jobType !== JobType.DISCOVER) {
      throw new BadRequestException(
        `Only discovery jobs can be updated with this endpoint. Job type: ${jobEntity.jobType}`,
      );
    }
  ...
  @ApiResponse({
    status: 200,
    description: 'Migration job configuration has been successfully updated.',
  })
  @ApiResponse({ status: 404, description: 'Job not found.' })
  @ApiResponse({
    status: 400,
    description:
      'Invalid job type - only migration jobs can be updated with this endpoint.',
  })
  @ApiResponse({
    status: 500,
    description: 'Internal Server Error - failed to update job configuration.',
  })
  @ApiBearerAuth()
  @Auth(Permission.ManageJob)
  ...
  async updateMigrationJobConfig(
    ...
    if (jobEntity.jobType !== JobType.MIGRATE) {
      throw new BadRequestException(
        `Only migration jobs can be updated with this endpoint. Job type: ${jobEntity.jobType}`,
      );
    }
    const jobConfigData: Partial<JobConfigDto> = {
      excludeFilePatterns: updateData.excludeFilePatterns,
  ...
  @ApiResponse({
    status: 200,
    description: 'Identity mappings fetched successfully',
  })
  @ApiResponse({
    status: 500,
  ...
  @ApiResponse({
    status: 200,
    description: 'Identity mappings deleted successfully',
  })
  @ApiResponse({
    status: 500,
  ...
  @ApiQuery({
    ...
    type: Boolean,
    description:
      'If true, recalculates stats from database. If false, returns cached stats. Default: false',
    example: false,
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 202,
    description:
      'Accepted - Calculation is in progress or no data available. Use fetch-latest=true to trigger calculation.',
  })
  @ApiResponse({
    status: 400,
    description: 'Bad Request - Invalid jobConfigID format.',
  })
  @ApiResponse({ status: 404, description: 'Job config not found.' })
  @ApiResponse({
    status: 500,
    description: 'Internal Server Error - failed to get inventory stats.',
  })
  @ApiBearerAuth()
  @Auth(Permission.ViewJob)
  ...
  async getJobConfigInventoryStats(
    ...
    @Query('fetch-latest', new ParseBoolPipe({ optional: true }))
    fetchLatest?: boolean,
  ): Promise<JobConfigInventoryStatsResponseDto> {
    const shouldFetchLatest = fetchLatest ?? false;
    return await this.jobConfigService.getJobConfigInventoryStats(
      jobConfigID,
      shouldFetchLatest,
    );
  }
  ...
  async getDirs(@Body() request: GetDirsDto): Promise<{ name: string }[]> {
    const fileServer = await this.jobConfigService.getFileServerById(
      request.fileServerId,
    );

    if (!fileServer) {
      throw new NotFoundException(
        `File server not found: ${request.fileServerId}`,
      );
    }
