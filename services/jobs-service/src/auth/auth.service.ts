...
export class AuthService implements OnModuleDestroy {
  ...
  constructor(
    ...
    this.logger.log(
      `[AuthService]: Keycloak endpoint: ${this.keycloakBaseUrl}/realms/${this.realm}/protocol/openid-connect/token`,
    );
  }
  ...
  async getAccessToken(force: boolean = false): Promise<string | null> {
    ...
    this.logger.log(
      `[AuthService]: getAccessToken called - force=${force}, hasToken=${!!this.accessToken}, now=${now}, expiresAt=${this.expiresAt}, isValid=${now < this.expiresAt}`,
    );

    // Return cached token if still valid (buffer already applied during storage)
    if (!force && this.accessToken && now < this.expiresAt) {
      this.logger.log(
        `[AuthService]: Returning cached token (expires in ${this.expiresAt - now}s)`,
      );
    ...
    try {
      ...
      this.logger.log(
        `[AuthService]: Fetched new access token, expires at: ${this.expiresAt} (in ${response.data.expires_in}s)`,
      );
  ...
  private scheduleTokenRefresh(_expiresIn: number): void {
    ...
    this.logger.log(
      `[AuthService]: Scheduling token refresh in ${refreshTime / 1000}s (${tokenRefreshMinutes / 60} hours)`,
    );

    this.tokenRefreshInterval = setTimeout(() => {
      this.logger.log('[AuthService]: Auto-refreshing JWT token...');
      void this.getAccessToken(true).catch((error: Error) => {
        this.logger.error(
          `[AuthService]: Failed to auto-refresh token: ${error.message}`,
        );
        // Retry in 30 seconds
        setTimeout(() => void this.getAccessToken(true), 30000);
      });
    }, refreshTime);
  }
}
