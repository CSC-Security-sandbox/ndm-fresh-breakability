...
describe("ErrorLogService", () => {
  ...
  describe("isCsvFileReady", () => {
    ...
    describe("createCsvFileForJob", () => {
      ...
      it("creates the file if it does not exist and cleans up .processing file", async () => {
        jest
          .spyOn(service, "getTotalErrorCountForJobRun")
          .mockResolvedValue(10);
        const dir = service.getErrorLogsDirectory;
        const fileName = "job10-error-10.csv";
        const filePath = path.join(dir, fileName);
        (fs.existsSync as any).mockImplementation((p) => {
          if (p === filePath) return false;
          if (p === dir) return true;
        ...
        jest
          ...
          .mockResolvedValueOnce([{ id: "1" }])
          .mockResolvedValueOnce([]);
        jest.spyOn(service, "getJobRunIds").mockResolvedValue(["run10"]);
        jest
          .spyOn(service as any, "fetchFormattedSetupErrors")
          .mockResolvedValue([{ id: "setup10" }]);
        await service.createCsvFileForJob("job10");
        expect(fs.createWriteStream).toHaveBeenCalledWith(filePath);
        expect(fs.writeFileSync).toHaveBeenCalledWith(
          `${filePath}.processing`,
          "processing"
        );
        expect(fs.unlinkSync).toHaveBeenCalledWith(`${filePath}.processing`);
      });

      it("handles writeLargeCsvToDisk with multiple pages", async () => {
        jest.spyOn(service, "getTotalErrorCountForJobRun").mockResolvedValue(6);
        const dir = service.getErrorLogsDirectory;
        const fileName = "job6-error-6.csv";
        const filePath = path.join(dir, fileName);
        (fs.existsSync as any).mockImplementation((p) => {
          if (p === filePath) return false;
          if (p === dir) return true;
        ...
        jest
          ...
          .mockResolvedValueOnce([]);
        jest.spyOn(service, "getJobRunIds").mockResolvedValue(["run6"]);
        jest
          .spyOn(service as any, "fetchFormattedSetupErrors")
          .mockResolvedValue([{ id: "setup6" }]);
        await service.createCsvFileForJob("job6");
        expect(fs.createWriteStream).toHaveBeenCalledWith(filePath);
        expect(fs.writeFileSync).toHaveBeenCalledWith(
          `${filePath}.processing`,
          "processing"
