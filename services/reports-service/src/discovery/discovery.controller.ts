...
export class DiscoveryController {
  ...
  @ApiResponse({
    status: 200,
    description: "Discovery inventory completed successfully",
  })
  @ApiResponse({
    status: 400,
    description: "Bad Request: Missing fileServerId",
  })
  async discoverFileServerDefault(
  ...
  async discoverFileServerWithPath(
    ...
    return await this.discoveryService.getDiscoveryByFileServerIdAndParentPath(
      fileServerId,
      parentPath
    );
  }
  ...
  @Post("/download")
  @ApiOperation({
    summary: "Download reports based on jobRunId and report type",
  })
  @ApiBody({
    schema: {
      type: "object",
  ...
  async downloadReports(
    ...
    if (!Object.values(ReportType).includes(reportType)) {
      throw new BadRequestException(
        "Invalid report type. Allowed values are COC or discovery"
      );
    }

    const zipBuffer = await this.discoveryService.getReportsAsZip(
      jobRunIds,
      reportType
    );
  ...
  @ApiResponse({
    status: 400,
    description: "Bad Request: Missing jobRunId or report-type",
  })
  ...
  async generateReport(
    ...
    this.logger.debug(
      `Generating report for jobRunId: ${jobRunId}, reportType: ${reportType}`
    );
    if (!jobRunId) {
  ...
  async generateDiscoveryReport(
    @Payload() payload: DiscoveryCompletedPayload,
