...
export class PdfService {
  ...
  async generatePdf(jobRunId: string, reportType: ReportType): Promise<Buffer> {
    this.logger.log(
      `Checking for existing report for jobRunId: ${jobRunId} and reportType: ${reportType}`
    );
  ...
  async generateJobsReportPdf(jobRunId: string): Promise<Buffer> {
    try {
      ...
      if (!data.length) {
        ...
        this.logger.error(
          `Report data not found for jobRunId: ${jobRunId} and reportType: JOBS_REPORT`
        );
        this.logger.log(
          `Calling discoveryService.createJobsPDFReportData for jobRunId: ${jobRunId}`
        );
        this.discoveryService.createJobsPDFReportData(jobRunId);
        this.logger.log(
          `Called discoveryService.createJobsPDFReportData for jobRunId: ${jobRunId}, try again later`
        );
        throw new HttpException(
      ...
      if (
        ...
        throw new Error("Invalid or missing summary data in reportData");
      }
      reportData.last_iteration.summary = reportData.summary[0];
      reportData.last_errors.summary = reportData.summary[0];
      reportData.cutovers =
        reportData.summary.filter(
          (item) => item.source.job_type === "CUT_OVER"
        ) ?? [];
      ...
      this.logger.error(
        `Failed to generate jobs report for jobRunId: ${jobRunId}, error: ${error}`
      );
      throw new HttpException(
