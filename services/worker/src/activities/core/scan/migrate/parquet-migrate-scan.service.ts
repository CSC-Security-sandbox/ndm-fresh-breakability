...
import { ParquetCommandGenerationService, LocalSetLookup } from "../../shared/parquet-command-generation.service";
...
@Injectable()
export class ParquetMigrateScanService {
    ...
    constructor(
        ...
        private readonly commandGenerationService: ParquetCommandGenerationService,
        ...
        this.logger = loggerFactory.create(ParquetMigrateScanService.name);
    }
    ...
    async scanDirectory({ jobContext, sourcePath, sourcePrefix, targetPath, command, settings, targetPrefix, errorType }: ScanDirectoryInput): Promise<ScanDirectoryOutput> {
        ...
        // Process items using parquet-enabled service — publishes to both command and parquet streams
