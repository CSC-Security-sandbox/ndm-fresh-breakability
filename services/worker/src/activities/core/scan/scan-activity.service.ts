...
export class ScanService {
    ...
    async updateAndReportTaskStatus({ errors, jobContext, taskHashId, task, retryCount }: UpdateAndReportTaskInput) {
        ...
        if (errors.some(isSourceFatalError)) {
            await jobContext.deleteTask(taskHashId);
            throw new FatalError(`Sync Task Update Failed: code ${[...new Set(errors)].length} ${errors.length} source errors with retry count ${retryCount} With Fatal Error`);
        }
        ...
        throw new RetryableError(`Sync Task Update Failed: code ${[...new Set(errors)].length} ${errors.length} source errors with retry count ${retryCount} With Retryable Error`);   
    }
