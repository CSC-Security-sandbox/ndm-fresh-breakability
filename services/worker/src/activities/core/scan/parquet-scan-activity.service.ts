...
import { ParquetMigrateScanService } from "./migrate/parquet-migrate-scan.service";
import { BatchSubDirInput, BatchSubDirOutput, ScanActivityInput, ScanActivityOutput, ScanDirectoryInput, ScanDirectoryOutput, TaskExecInput, TaskExecOutput, UpdateAndReportTaskInput } from './scan-activity.type';
import { calculateHash } from "src/activities/utils/checksum-utils";


@Injectable()
export class ParquetScanService {
    ...
    constructor(
        ...
        private readonly migrateScanService: ParquetMigrateScanService,
    ) {
    ...
    async scanDirectories({ jobRunId, batchSize, batchId }: ScanActivityInput): Promise<ScanActivityOutput> {
    ...
    async executeTask({ activityId, jobContext, jobRunId, task, batchSize }: Omit<TaskExecInput, 'isMigration'>): Promise<TaskExecOutput> {
        ...
        for (let i = 0; i < task.commands.length; i += this.maxConcurrency) {
            ...
            await Promise.allSettled(
                batch.map(async (command) => {
                    ...
                    try {
                        const result: ScanDirectoryOutput = await this.migrateScanService.scanDirectory(scanDirectoryInput);
