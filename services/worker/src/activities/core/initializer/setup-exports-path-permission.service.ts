...
export class SetupExportsPathPermissionService {
    ...
    async setupExportPathPermission(jobRunId: string) {
        ...
        const sourcePath = basePrefix(jobRunId,jobContext.jobConfig?.sourceFileServer?.pathId);
        const targetPath = basePrefix(jobRunId,jobContext.jobConfig?.destinationFileServer?.pathId);

        this.logger.log(`Setting up export path permission from ${sourcePath} to ${targetPath}`);
        try {
            const sourceAcl: SecurityDescriptor = await this.winOperationService.getAclOperation(sourcePath, false);
            const targetAcl: SecurityDescriptor = await this.winOperationService.getAclOperation(targetPath, true);

            await this.winOperationService.setAclOperation(targetPath, sourceAcl);
