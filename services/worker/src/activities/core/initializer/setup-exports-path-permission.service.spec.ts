...
describe('SetupExportsPathPermissionService', () => {
  ...
  describe('setup', () => {
    ...
    it('should throw when setAclOperation returns unresolved SIDs', async () => {
      const jobRunId = 'test-job-run-id';
      const sourceAcl = makeSourceAcl();
      const context = {
        jobConfig: {
      ...
      mockWinOperationService.setAclOperation.mockResolvedValue({
        stdout: JSON.stringify({ success: true, unresolved_sids: ['S-1-5-21-000', 'S-1-5-21-999'] }),
      });

      await expect(service.setup(jobRunId, context)).rejects.toThrow('Unresolved SIDs');
    });

    it('should throw when setAclOperation returns success false', async () => {
      const jobRunId = 'test-job-run-id';
      const sourceAcl = makeSourceAcl();
      const context = {
        jobConfig: {
      ...
      await expect(service.setup(jobRunId, context)).rejects.toThrow('Stamp failed: Access denied');
    });

    it('should throw with combined message when both unresolved SIDs and stamp failure occur', async () => {
      const jobRunId = 'test-job-run-id';
      const sourceAcl = makeSourceAcl();
      const context = {
        jobConfig: {
      ...
      mockWinOperationService.getAclOperation.mockResolvedValueOnce(sourceAcl);
      mockWinOperationService.setAclOperation.mockResolvedValue({
        stdout: JSON.stringify({ success: false, error: 'partial failure', unresolved_sids: ['S-1-5-21-000'] }),
      });

      await expect(service.setup(jobRunId, context)).rejects.toThrow('Unresolved SIDs: S-1-5-21-000; Stamp failed: partial failure');
    });

    it('should handle unparseable stdout from setAclOperation gracefully', async () => {
      const jobRunId = 'test-job-run-id';
      const sourceAcl = makeSourceAcl();
      const context = {
        jobConfig: {
      ...
      mockWinOperationService.getAclOperation
        .mockResolvedValueOnce(sourceAcl)
        .mockResolvedValueOnce(sourceAcl);
      mockWinOperationService.setAclOperation.mockResolvedValue({ stdout: 'not-valid-json' });
      mockWinOperationService.validateAclOperation.mockResolvedValue({ sourceSID: '', targetSID: '', inValid: '' });

      await service.setup(jobRunId, context);

      expect(mockLogger.error).toHaveBeenCalledWith(
        expect.stringContaining('Failed to parse Set ACL result')
      );
    });
  });
});
