...
export interface DnsConfigLogger {
 ...
 */
export async function configureSmartConnectDns(
  traceId: string,
  fileServer: SmartConnectFileServer,
  logger: DnsConfigLogger
): Promise<boolean> {
  ...
  logger.log(`[${traceId}] Configuring SmartConnect DNS: SSIP=${ssip}, Zone=${dnsZone}, Platform=${process.platform}`);

  try {
    switch (process.platform) {
      case 'linux':
        await configureLinuxDns(traceId, ssip, dnsZone, logger);
        break;
      case 'darwin':
        await configureMacOSDns(traceId, ssip, dnsZone, logger);
        break;
      case 'win32':
        await configureWindowsDns(traceId, ssip, dnsZone, logger);
        break;
      default:
    ...
    logger.error(`[${traceId}] Failed to configure SmartConnect DNS: ${error.message}`);
    throw error;
  }
}

async function configureLinuxDns(
  traceId: string,
  ssip: string,
  dnsZone: string,
  logger: DnsConfigLogger
  ...
  const currentContent = await readFileOrEmpty(resolvConfPath, traceId, logger);

  if (currentContent.includes(nameserverEntry)) {
    logger.log(`[${traceId}] SmartConnect SSIP ${ssip} already configured`);
    return;
  }

  const newContent = buildResolvConf(currentContent, ssip, dnsZone);
  await fsPromises.writeFile(resolvConfPath, newContent);

  logger.log(`[${traceId}] Linux: SmartConnect DNS configured successfully`);
}

async function configureMacOSDns(
  traceId: string,
  ssip: string,
  dnsZone: string,
  logger: DnsConfigLogger
  ...
  try {
    ...
    if (content.includes(ssip)) {
      logger.log(`[${traceId}] SmartConnect SSIP ${ssip} already configured for ${dnsZone}`);
      return;
    }
  } catch {
    logger.log(`[${traceId}] Resolver file not found for ${dnsZone}, creating new one`);
  }
  ...
  logger.log(`[${traceId}] macOS: SmartConnect DNS configured at ${resolverFile}`);
}

async function configureWindowsDns(
  traceId: string,
  ssip: string,
  dnsZone: string,
  logger: DnsConfigLogger
): Promise<void> {
  if (await isWindowsDnsConfigured(traceId, ssip, dnsZone, logger)) {
    return;
  }
  ...
  try {
    ...
    logger.log(`[${traceId}] Windows: SmartConnect DNS NRPT rule added for ${dnsZone} -> ${ssip}`);
  } catch (addError) {
    logger.warn(`[${traceId}] Failed to add NRPT rule: ${addError.message}. Trying netsh fallback...`);
    await configureWindowsDnsViaNetsh(traceId, ssip, logger);
  }
}

async function isWindowsDnsConfigured(
  traceId: string,
  ssip: string,
  dnsZone: string,
  logger: DnsConfigLogger
  ...
  try {
    ...
    if (stdout && stdout.trim()) {
      logger.log(`[${traceId}] SmartConnect DNS rule already exists for ${dnsZone}`);
      return true;
    }
  } catch (checkError) {
    logger.warn(`[${traceId}] PowerShell NRPT check failed: ${checkError.message}. Trying nslookup fallback...`);
    ...
    try {
      ...
      if (nslookupOut && !nslookupOut.includes("can't find") && !nslookupOut.includes('NXDOMAIN')) {
        logger.log(`[${traceId}] SmartConnect DNS zone ${dnsZone} already resolves via ${ssip}`);
        return true;
      }
    } catch {
      logger.log(`[${traceId}] SmartConnect DNS rule not found for ${dnsZone}, creating new rule`);
    }
  }
...
async function configureWindowsDnsViaNetsh(
  traceId: string,
  ssip: string,
  logger: DnsConfigLogger
): Promise<void> {
  ...
  try {
    ...
    logger.log(`[${traceId}] Windows: SmartConnect DNS added via netsh`);
  } catch (netshError) {
...
async function readFileOrEmpty(
  ...
  try {
    ...
    logger.warn(`[${traceId}] Could not read ${filePath}, will create new file`);
    return '';
  }
}

function buildResolvConf(currentContent: string, ssip: string, dnsZone: string): string {
