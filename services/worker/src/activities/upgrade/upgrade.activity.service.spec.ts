...
describe('UpgradeActivityService', () => {
  ...
  describe('downloadBundle', () => {
    it('should delegate to handler with heartbeat callback', async () => {
      ...
      const result = await service.downloadBundle({ version: '1.0.0' });

      expect(result).toEqual(mockOutput);
      expect(mockHandler.download).toHaveBeenCalledWith('1.0.0', expect.any(Function));
    });

    it('should propagate handler errors', async () => {
      mockHandler.download.mockRejectedValue(new Error('Download failed'));

      await expect(service.downloadBundle({ version: '1.0.0' })).rejects.toThrow('Download failed');
    });
  });
  ...
  describe('ackUpgrade', () => {
    it('should POST ack with auth token', async () => {
      ...
      await service.ackUpgrade({ version: '1.0.0', status: 'success' });
    ...
    it('should POST ack without auth when token is null', async () => {
      ...
      await service.ackUpgrade({ version: '1.0.0', status: 'failed', message: 'error' });
    ...
    it('should not throw when POST fails', async () => {
      ...
      await expect(
        service.ackUpgrade({ version: '1.0.0', status: 'success' }),
