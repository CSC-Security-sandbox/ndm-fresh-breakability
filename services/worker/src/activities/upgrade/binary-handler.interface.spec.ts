...
describe('BaseBinaryHandler', () => {
  ...
  describe('cleanupMacOSJunk', () => {
    it('should remove .DS_Store and ._ files', async () => {
      mockedFs.readdir.mockResolvedValue(['.DS_Store', '._binary', 'real-file'] as any);
      mockedFs.unlink.mockResolvedValue(undefined);

      await (handler as any).cleanupMacOSJunk('/dir');

      expect(mockedFs.unlink).toHaveBeenCalledTimes(2);
      expect(mockedFs.unlink).toHaveBeenCalledWith(expect.stringContaining('.DS_Store'));
      expect(mockedFs.unlink).toHaveBeenCalledWith(expect.stringContaining('._binary'));
    });

    it('should not remove regular files', async () => {
      mockedFs.readdir.mockResolvedValue(['binary', '.env'] as any);
