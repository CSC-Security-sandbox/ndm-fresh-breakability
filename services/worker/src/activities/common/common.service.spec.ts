...
describe('CommonActivityService', () => {
  ...
  beforeEach(async () => {
    ...
    mockContext = {
      ...
      groupReadTasks: jest
        .fn()
        .mockResolvedValue([Promise.resolve({ id: 't1' })]),
      groupReadMigrationTask: jest
        .fn()
        .mockResolvedValue([Promise.resolve({ id: 'mt1' })]),
  ...
  describe('updateStatus', () => {
    it('should update status successfully', async () => {
      ...
      expect(res).toEqual({
        message: 'Job status updated for job id: ' + jobRunId,
      });
    });
    ...
    it('should handle missing token', async () => {
      ...
      await expect(
        service.updateStatus({ jobRunId, status: JobRunStatus.Completed }),
      ).rejects.toThrow(
        'Error while updating the status of the job id : job123',
      );
  ...
  describe('generateJobsReport', () => {
    it('should trigger report successfully', async () => {
      ...
      expect(res).toEqual({
        message:
          'Triggering generateJobsReport successful for job id: ' + jobRunId,
      });
    });
    ...
    it('should handle missing token', async () => {
      ...
      await expect(service.generateJobsReport(jobRunId)).rejects.toThrow(
        'Error while Triggering generateJobsReport for the job id : job123',
      );
  ...
  describe('updateJobErrorStatus', () => {
    it('should call updateStatus and updateLastEntry', async () => {
      ...
      const spyLast = jest
        .spyOn(service, 'updateLastEntry')
        .mockResolvedValue({ message: 'ok' });

      await service.updateJobErrorStatus(jobRunId);
      expect(spyStatus).toHaveBeenCalledWith({
        jobRunId,
    ...
    it('should propagate errors from updateStatus', async () => {
      ...
      await expect(service.updateJobErrorStatus(jobRunId)).rejects.toThrow(
    ...
    it('should propagate errors from updateLastEntry', async () => {
      ...
      jest
        .spyOn(service, 'updateLastEntry')
        .mockRejectedValue(new Error('Last entry failed'));
  ...
  describe('updateLastEntry', () => {
    ...
    it('should throw error when Redis operations fail', async () => {
      ...
      (redisService.getJobManagerContext as jest.Mock).mockRejectedValueOnce(
        error,
      );

      await expect(service.updateLastEntry(traceId)).rejects.toThrow(
        'Error while marking the job as completed : test-trace',
      );
  ...
  describe('generateCOCReport', () => {
    it('should trigger COC report successfully', async () => {
      ...
      expect(res).toEqual({
        message:
          'Triggering generateCOCReport successful for job id: ' + jobRunId,
      });
    });
    ...
    it('should throw error for missing token', async () => {
      ...
      await expect(service.generateCOCReport(jobRunId)).rejects.toThrow(
        'Error while Triggering generateCOCReport for the job id : job123',
      );
    });
  ...
  describe('generateDiscoveryReport', () => {
    it('should trigger discovery report successfully', async () => {
      ...
      expect(res).toEqual({
        message:
          'Trigger generateDiscoveryReport Successful for job id: ' + jobRunId,
      });
    });
    ...
    it('should throw error for missing token', async () => {
      ...
      await expect(service.generateDiscoveryReport(jobRunId)).rejects.toThrow(
        'Error while Trigger generateDiscoveryReport the status of the job id : job123',
      );
    });
  ...
  describe('cleanupJobContext', () => {
    ...
    it('should throw error when cleanup fails', async () => {
      ...
      await expect(service.cleanupJobContext(traceId)).rejects.toThrow(
        'Error while cleaning up the job context: test-trace',
      );
  ...
  describe('updateCutOverStatus', () => {
    it('should update cutover status successfully', async () => {
      ...
      const res = await service.updateCutOverStatus({
        jobRunId,
  ...
  describe('updateWorkerResponse', () => {
    it('should update worker response successfully', async () => {
      (axios.put as jest.Mock).mockResolvedValue({});
      const res = await service.updateWorkerResponse(jobRunId, workerId, {
        data: 1,
      });
      expect(res).toEqual({
        message: 'Worker response updated successfully for job id: ' + jobRunId,
      });
    });
    ...
    it('should handle error', async () => {
      ...
      await expect(
        service.updateWorkerResponse(jobRunId, workerId, { data: 1 }),
      ).rejects.toThrow(
        'Error while updating the worker response for the job id : job123',
      );
  ...
  describe('Configuration scenarios', () => {
    it('should handle missing config values gracefully', async () => {
      ...
      const serviceWithMissingConfig = module.get<CommonActivityService>(
        CommonActivityService,
      );
  ...
  describe('Error handling edge cases', () => {
    it('should handle axios errors with detailed error messages', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(
        `[${jobRunId}] Failed to update status: ${detailedError}`,
      );
    });
    ...
    it('should handle network errors for generateCOCReport', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(
        expect.stringContaining('Failed to Trigger generateCOCReport'),
      );
    });
  });
  ...
  describe('Integration scenarios', () => {
    ...
    it('should handle cleanup failures with detailed error logging', async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(
        `[${traceId}] Error while cleaning up the job context: ${cleanupError}`,
      );
    });
  });
});
