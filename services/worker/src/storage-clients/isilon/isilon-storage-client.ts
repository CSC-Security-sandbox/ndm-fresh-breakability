...
// ==========================================
// Stub types for worker service (no TypeORM dependency)
// These mirror the config-service entities/DTOs for future library extraction
// ==========================================

/** Stub for TypeORM Repository - provides minimal interface needed */
export interface Repository<T> {
  findOne(options: { where: { id: string }; relations?: string[] }): Promise<T | null>;
}

/** Stub for @InjectRepository decorator - no-op in worker */
export function InjectRepository(entity: any): ParameterDecorator {
  return () => {};
}

/** Stub ConfigEntity interface */
...
/** DTO for fetching zones response */
...
/** Zone DTO */
export interface IsilonZoneDTO {
  zoneId: number;
  zoneName: string;
  ipAddresses: string[];
  smartConnectFqdn?: string | null;
  ssip?: string | null;
  scDnsZone?: string | null;
}

/** NFS Export Path DTO */
export interface NFSExportPathDTO {
  path: string;
  id?: number;
}

/** SMB Share DTO */
export interface SMBShareDTO {
  name: string;
  path?: string;
}
...
export class IsilonStorageClient extends StorageClient {
  ...
  constructor(
    ...
    @Optional() @InjectRepository(FileServerEntity)
