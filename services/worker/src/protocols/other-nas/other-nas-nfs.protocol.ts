...
import { handleConnectionError, parseExports, parseProtocolVersions } from '../nfs/nfs.utils';
...
/**
 * Generic NAS NFS Protocol
 * Handles standard NFS operations using showmount and standard NFS commands
 */
@Injectable()
export class OtherNasNfsProtocol extends Protocol {
  ...
  public updateBootMounts({ platform, fstabPath, workerId }, payload, action, traceId){
    this.logger.log(`[${traceId}] Updating boot mounts for platform: ${platform}`);
    // Implementation for updating fstab/autofs entries
     try {
          const mountDir = `${payload.mountBasePath}/${payload.jobRunId}/${payload.pathId}`;
