    ...
    async collectNetworkLatency() {
        try {
            ...
            if (elapsedMinutes >= 10) {
                this.logger.log(`elapsedMinutes time ${new Date().toLocaleString()}`);
    ...
    private async sendPendingBatches() {
        ...
        for (const batch of this.pendingBatches) {
            try {
                ...
                if (!accessToken) {
                    this.logger.error('Unable to get access token for authentication');
                    stillPending.push(batch);
                    continue;
                }

                await axios.post(
                    `${this.workerConfigUrl}/api/v1/support-bundle/network-latency`,
                    { measurements: batch },
                    {
                        timeout: 10000,
                        headers: {
