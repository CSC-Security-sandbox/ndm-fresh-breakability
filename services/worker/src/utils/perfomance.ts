...
export function LogExecutionTime<T extends (...args: any[]) => any>(
  ...
  if (propertyKey && descriptor) {
    ...
    descriptor.value = function (...args: any[]) {
      ...
      logToFile(
        `[TIMING] ${className}.${propertyKey} - Start time: ${new Date().toISOString()}`,
      );
      ...
      if (result instanceof Promise) {
        return result
          .then((res) => {
            ...
            logToFile(
              `[TIMING] ${className}.${propertyKey} - End time: ${new Date().toISOString()}, Total execution time: ${executionTime}ms`,
            );
            return res;
          })
          .catch((error) => {
            const executionTime = (Date.now() - start).toFixed(3);
            logToFile(
              `[TIMING] ${className}.${propertyKey} - End time (error): ${new Date().toISOString()}, Total execution time: ${executionTime}ms`,
            );
            throw error;
          });
      } else {
  ...
  // target is actually the function in this case
    ...
    const wrappedFunction = function (...args: any[]) {
      const start = Date.now();
      logToFile(
        `[TIMING] ${functionName} - Start time: ${new Date().toISOString()}`,
      );
      ...
      if (result instanceof Promise) {
        return result
          .then((res) => {
            const executionTime = (Date.now() - start).toFixed(3);
            logToFile(
              `[TIMING] ${functionName} - End time: ${new Date().toISOString()}, Total execution time: ${executionTime}ms`,
            );
            return res;
          })
          .catch((error) => {
            const executionTime = (Date.now() - start).toFixed(3);
            logToFile(
              `[TIMING] ${functionName} - End time (error): ${new Date().toISOString()}, Total execution time: ${executionTime}ms`,
            );
            throw error;
          });
      } else {
