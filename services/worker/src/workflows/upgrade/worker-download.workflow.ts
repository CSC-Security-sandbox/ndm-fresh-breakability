 ...
 *   - Binary: datamigrator-worker-{platform}-{version}[.exe]
 *   - Env:    datamigrator-worker-{platform}-{version}.env
 *   - Checksums: datamigrator-worker-{platform}-{version}.sha256
 * 
 * Flow:
...
export async function WorkerDownloadWorkflow(
  ...
  log.info(`[${traceId}] WorkerDownloadWorkflow starting for worker ${workerId}, bundle ${bundleId}`);
  ...
  try {
    ...
    if (staged) {
      ...
      await ackUpgrade({ bundleId, version, status: 'success' });
      return {
        workerId,
        platform,
        status: 'success',
    ...
    const result = await downloadBundle({ bundleId, version });
    ...
    try {
      await ackUpgrade({ bundleId, version, status: 'failed', message: errorMessage });
    } catch {
      log.error(`[${traceId}] Failed to send failure ack: ${errorMessage}`);
    }
