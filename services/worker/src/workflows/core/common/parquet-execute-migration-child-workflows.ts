...
interface ParquetMigrationWorkflowExecutorInput {
    jobRunId: string;
}

interface ParquetMigrationWorkflowExecutorOutput {
    status: JobRunStatus;
    fileCount: number;
    dirCount: number;
...
export const executeParquetMigrationChildWorkflows = async ({ jobRunId }: ParquetMigrationWorkflowExecutorInput): Promise<ParquetMigrationWorkflowExecutorOutput> => {

    let scanWorkflow: wf.ChildWorkflowHandle<wf.Workflow>, syncWorkflow: wf.ChildWorkflowHandle<wf.Workflow>;
    let stopRequested = false;
    let output: ParquetMigrationWorkflowExecutorOutput = {
        ...
        if (action == JobRunStatus.Stopped) {
            ...
            try {
                ...
                await updateWorkerResponse(jobRunId, 'all', {
                    ...
                    occurrence: 1,
                    origin: 'ParquetMigrationWorkflow',
    ...
    if (!stopRequested) {
        ...
        scanWorkflow = await wf.startChild('ParquetChildScanWorkflow', {
            args: [{ jobRunId: jobRunId, workerConcurrency, batchSize }],
            workflowId: `ParquetScanWorkflow-${jobRunId}`,
    ...
    if (!stopRequested) {
        syncWorkflow = await wf.startChild('ParquetChildSyncWorkflow', {
            args: [{ jobRunId: jobRunId, scanWorkflowStatus: JobRunStatus.Running, actionState: JobRunStatus.Running }],
            workflowId: `ParquetSyncWorkflow-${jobRunId}`,
            taskQueue: `${jobRunId}-TaskQueue`,
            cancellationType: wf.ChildWorkflowCancellationType.WAIT_CANCELLATION_COMPLETED,
            parentClosePolicy: wf.ParentClosePolicy.TERMINATE,
        });
    ...
    if (scanWorkflow && syncWorkflow) {
        ...
        const scanPromise = scanWorkflow.result()
            ...
            .catch(async (error) => {
                output.scanJobStatus = await handleError(error, syncWorkflow.workflowId, 'ParquetChildScanWorkflow', 'Scan Workflow Failed');
            });
        ...
        const syncPromise = syncWorkflow.result()
            ...
            .catch(async (error) => {
                output.syncJobStatus = await handleError(error, scanWorkflow.workflowId, 'ParquetChildSyncWorkflow', 'Sync Workflow Failed');
            });
