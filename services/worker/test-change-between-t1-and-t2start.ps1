# test-change-between-t1-and-t2start.ps1
# Simulates a single external ACL change on source between T1 and T2Start.
# Fires only on attempt 1 so the retry succeeds and stamps the new ACL.
# Adds Read permission for user 'kiran'. Does NOT restore.
#
# Expected result:
#   Attempt 1: T2Start > T1 → conflict detected (changedDuringStamp=true)
#   Attempt 2: no change → stamp succeeds with kiran Read on destination
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File test-change-between-t1-and-t2start.ps1 `
