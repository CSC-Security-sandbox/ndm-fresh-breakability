                ...
                <Box className="mb-4 p-4 bg-gray-50 rounded-lg border-l-4 border-l-orange-400">
                  ...
                  <Box className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                    ...
                    <Show>
                      <Show.When
                        isTrue={
                          conflict.jobRunIds && conflict.jobRunIds.length > 0
                        }
                      >
                        <Box className="md:col-span-2 flex items-center gap-2">
                          <span className="font-medium">
                            Active Job Runs ({conflict.jobRunIds.length}):
                          </span>
                          <Box className="flex flex-wrap gap-2">
                            <RenderEach
                              renderList={conflict.jobRunIds}
                              renderItem={(jobRunId) => (
                                <Box
                                  className="px-2 py-1 bg-gray-200 rounded text-xs font-mono cursor-pointer hover:bg-gray-300 transition-colors"
                                  title={`Click to copy: ${jobRunId}`}
                                >
                                  {jobRunId?.substring(0, 8)}...
                                </Box>
                              )}
                            />
                          </Box>
