...
// import { VirtualMachineIcon } from "@netapp/bxp-style/react-icons/Storage";
import RefreshButton from "@components/refresh-button/RefreshButton";

// Hardcoded Dell Isilon zones (same as Credentials.tsx - should be moved to a shared constant)
...
const ValidateConnection = () => {
  ...
  const selectedZonesWithDetails = useMemo(() => {
    return safeSelectedZoneIds.map((zoneId) => {
      const zone = DELL_ISILON_ZONES.find((z) => z.id === zoneId);
  ...
  // Handle worker selection for Dell Isilon zones
  const handleZoneWorkerSelection = (workerIds: string[]) => {
    if (!activeZoneId) return;

    // Separate NFS and SMB workers
    const nfsWorkers = workerIds.filter((id) => {
      const worker = safeAllWorkersList.find((w) => w.workerId === id);
      return worker?.workerName?.toLowerCase().startsWith("nfs-");
    });
    const smbWorkers = workerIds.filter((id) => {
      const worker = safeAllWorkersList.find((w) => w.workerId === id);
      return worker?.workerName?.toLowerCase().startsWith("smb-");
    });

    setZoneWorkerAssignments((prev) => ({
      ...prev,
      [activeZoneId]: { nfs: nfsWorkers, smb: smbWorkers },
    }));
  };
  ...
  // Sync selection state with zone worker assignments
  useEffect(() => {
    if (isDellIsilon && activeZoneId && dellIsilonTableState?.selectionState) {
      const selectedRows = Object.keys(dellIsilonTableState.selectionState.rows || {}).filter(
        (key) => dellIsilonTableState.selectionState.rows[key] === true
      );
      handleZoneWorkerSelection(selectedRows);
    }
  }, [isDellIsilon, activeZoneId, dellIsilonTableState?.selectionState?.count]);
  ...
  if (isDellIsilon) {
    const { pagination, selectionState, rows: tableRows, sortState, toggleSort, filterState, updateFilterState, toggleRowSelection, updateTextFilter } = dellIsilonTableState;
