...
export const flattenDellIsilonPayloadToConfigs = (
  ...
  // Build the management server object (same for all configs from this Dell Isilon)
  const managementServer: ManagementServerType = {
    projectId: dellPayload.projectId,
    configName: dellPayload.parentName,
    serverType: "Dell",
    host: dellPayload.managementHost,
    username: dellPayload.managementUsername,
    password: dellPayload.managementPassword,
  ...
  for (const zone of dellPayload.zones) {
    ...
    if (zone.nfs) {
      const nfsConfig: ConfigPayloadType = {
        ...
        // Management server (required for Dell Isilon)
        managementServer,
