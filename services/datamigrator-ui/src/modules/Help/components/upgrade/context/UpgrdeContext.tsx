...
export const UpgradeProvider = ({ children }: React.PropsWithChildren) => {
  ...
  useEffect(() => {
    if (latestStatus) {
      ...
      // Show filename for processing state
      setInProgressFileName(latestStatus.isProcessing ? (latestStatus.fileName || '') : '');
  ...
  const validateFile = (file: File): { valid: boolean; error?: string } => {
    ...
    if (file.size === 0) {
      return { 
        valid: false, 
        error: 'File is empty. Please select a valid upgrade bundle.' 
      };
    }
  ...
  // Reset everything - also marks upgrade as skipped if there was a successful upload
  const handleReset = async () => {
    ...
    if (uploadProgress.bundleId && uploadProgress.status === 'uploaded') {
      try {
        ...
        // Continue with reset even if skip fails - don't block the user
      }
    }
