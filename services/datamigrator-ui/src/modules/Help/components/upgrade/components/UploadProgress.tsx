...
const UploadProgress = () => {
  ...
  const getStatusMessage = () => {
    ...
    // For error/validation_failed status, just return base message (errors shown separately)
    if (uploadProgress.status === "error" || uploadProgress.status === "validation_failed") {
      return baseMessage;
    }
  ...
  return (
    <Box className="mt-6 p-4 bg-gray-50 rounded">
      ...
      {/* Error Details Box - shown for both error and validation_failed */}
      {(uploadProgress.status === "error" || uploadProgress.status === "validation_failed") && errorLines.length > 0 && (
        <Box className="mt-3 p-3 bg-red-50 border border-red-200 rounded max-h-48 overflow-y-auto">
          <p className="text-sm font-medium text-red-800 mb-2">
            {uploadProgress.status === "validation_failed" ? "Missing Files" : "Errors"} ({errorLines.length}):
          </p>
