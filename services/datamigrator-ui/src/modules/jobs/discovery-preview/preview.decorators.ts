...
export function chartDataForFileDepth(jsonData: DataItemType[]): ProcessedData {
  return extractChartData(jsonData, { category: FileSystemCategory.DEPTH });
}
...
export function longestFileNames(
  ...
  const longestFileEntry = jsonData?.find(
    (item) =>
      ...
      item.sub_category === FileSystemSubCategory.TOP_5_LONGEST_FILE_NAMES,
  );
...
export function extractLongestDirectoryPaths(data: DataItemType[]): FileInfo[] {
  ...
  let longestPathCounts = (longestDirPaths?.value as string)
    ...
    ?.map((entry: string) => {
      const match = entry.match(
        getRegExp(StringComparisonPattern.DIRECTORY_NAME_SIZE),
      );
      if (match) {
        return {
          fileName: match[1],
...
export function extractMaxAvgFileSize(data: DataItemType[]): {
  ...
  const avgFileSize =
    parseInt(totalSpaceUsed as string) / parseInt(totalCount as string);
  return {
    maxFileSize,
    avgFileSize,
  };
...
export function extractSystemFileStatAndDirectories(data: DataItemType[]) {
  ...
  const jobRunStatus =
    data?.find(
      (item: DataItemType) =>
        item.sub_category === FileSystemSubCategory.STATUS,
    )?.value || 0;
  const scanTime =
    data?.find(
      (item: DataItemType) =>
...
export function formatLargeNumber(num: number, decimals = 2): string {
  ...
  const i = Math.floor(Math.log10(num) / Math.log10(NUMBER_IN_KILOBYTE));

  const formattedNumber = (num / Math.pow(NUMBER_IN_KILOBYTE, i)).toFixed(
    decimals,
  );
