  ...
  const fetchAsupSettings = async () => {
    try {
      ...
      if (asupSettingsResult) {
        console.log("ASUP settings from backend", asupSettingsResult);
        dispatch(syncAsupSettings({
          enabled: asupSettingsResult.enabled,
          consentGiven: asupSettingsResult.consentGiven,
          lastUpdated: asupSettingsResult.lastUpdated,
          lastTransmission: asupSettingsResult.lastTransmission,
        }));
      }
    } catch (error) {
      console.error("Failed to fetch ASUP settings:", error);
