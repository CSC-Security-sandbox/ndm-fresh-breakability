...
export class OperationErrorService {
    ...
    /**
     * Get operation errors by operation IDs
     */
    async findByOperationIds(operationIds: string[]): Promise<OperationErrorEntity[]> {
        const query = `
      SELECT * FROM datamigrator.operation_errors 
      WHERE operation_id = ANY($1)
      ORDER BY created_at DESC
    `;
        return await this.operationErrorRepo.query(query, [operationIds]);
    }

    /**
     * Get operation errors count by project and date range
     */
    ...
    `;
        ...
        return await this.operationErrorRepo.query(query, [projectIds, startDate, endDate]);
    }
}
