...
describe('WorkerProviders', () => {
  ...
  describe('useFactory function', () => {
    ...
    it('should bind activities correctly', async () => {
      ...
      expect(mockActivitiesService.fetchAndZipLogs).toHaveBeenCalledWith(
        ...testArgs,
      );

      activities.notifyWorkflowCompletion(...testArgs);
      expect(
        mockActivitiesService.notifyWorkflowCompletion,
      ).toHaveBeenCalledWith(...testArgs);

      activities.getJobConfigIdsByProjectIds(...testArgs);
      expect(
        mockActivitiesService.getJobConfigIdsByProjectIds,
      ).toHaveBeenCalledWith(...testArgs);

      activities.generateErrorCsv(...testArgs);
      expect(mockActivitiesService.generateErrorCsv).toHaveBeenCalledWith(
        ...testArgs,
      );
    });
