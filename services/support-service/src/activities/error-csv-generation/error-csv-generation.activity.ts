...
import {
  formatDateTime,
  getProjectIds,
  groupDataByProjectAndDate,
} from 'src/utils/error-csv-generation.util';
...
export class ErrorCsvGenerationActivity {
  ...
  async generateErrorCsv({ traceId, payload }): Promise<ExportResult> {
    ...
    this.logger.log(
      `[${traceId}] Starting error CSV generation for date range: ${payload.startDate} to ${payload.endDate}`,
    );
    ...
    try {
      ...
      if (projectIds.length === 0) {
        this.logger.warn(
          `[${traceId}] No valid project IDs found in projectWorkerMap`,
        );
        return {
          success: true,
      ...
      this.logger.log(
        `[${traceId}] Processing ${projectIds.length} projects: [${projectIds.join(', ')}]`,
      );
    } catch (error) {
    ...
    try {
      ...
      if (data.length === 0) {
        this.logger.log(
          `[${traceId}] No operation errors found for the given criteria`,
        );
        return {
          success: true,
          message:
      ...
      this.logger.log(
        `[${traceId}] Found ${data.length} operation errors to process`,
      );
      ...
      this.logger.log(
        `[${traceId}] Successfully created ${filesCreated} CSV files`,
      );
      ...
      this.logger.error(
        `[${traceId}] Error exporting operation errors:`,
        error,
      );
      return {
        success: false,
  ...
  async getOperationErrorsByProjectAndDateRange(
    ...
    try {
      ...
      throw new Error(
        `Failed to fetch operation errors from database: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
   ...
   */
  async exportOperationErrorsToZip(
    request: ExportRequest,
    traceId: string,
  ): Promise<void> {
    this.logger.log(
      `[${traceId}] Starting CSV export to zip for ${request.projectIds.length} projects`,
    );
    ...
    try {
      ...
      if (data.length === 0) {
        this.logger.warn(
          `[${traceId}] No operation errors found for export criteria`,
        );
        return;
      }
      ...
      if (groupedData.size === 0) {
        this.logger.warn(
          `[${traceId}] No grouped data by project ID and date available after processing ${data.length} records for error CSV generation`,
        );
        return;
      }

      this.logger.log(
        `[${traceId}] Grouped ${data.length} errors into ${groupedData.size} project groups`,
      );
      ...
      this.logger.error(
        `[${traceId}] Failed to export operation errors to zip:`,
        error,
      );
      throw new Error(
        `Failed to export errors to zip: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private async addCSVFilesToZip(
    ...
    try {
      ...
      throw new Error(
        `Failed to load existing zip file: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    ...
    this.logger.log(
      `[${traceId}] Found ${zipEntries.length} entries in zip file`,
    );
    ...
    if (zipEntries.length === 0) {
      ...
      this.logger.log(
        `[${traceId}] Zip contains ${dirCount} directories and ${fileCount} files.`,
      );
    }
    ...
    for (const [projectId, dateGroups] of groupedData.entries()) {
      ...
      for (const [date, errors] of dateGroups.entries()) {
        ...
        const formattedDate = date.includes('/')
          ...
          : date;
        this.logger.log(
          `[${traceId}] Processing: Project '${projectId}', Date '${formattedDate}' (${errors.length} errors)`,
        );
    ...
    this.logger.log(
      `[${traceId}] Zip file updated successfully: ${zipFilePath}`,
    );
  ...
  private async addCSVToZip(
    ...
    try {
      this.logger.log(
        `[${traceId}] Looking for existing location for project '${projectId}' and date '${date}'`,
      );
      ...
      zipEntries.forEach((entry) => {
        if (entry.isDirectory) {
          ...
          const dirPath = entry.entryName.substring(
            0,
      ...
      this.logger.log(
        `[${traceId}] Discovered ${allPaths.size} unique paths in ZIP`,
      );
      ...
      if (!foundLocation) {
        this.logger.log(
          `[${traceId}] No suitable location found for project '${projectId}' and date '${date}' - skipping`,
        );
      ...
      this.logger.log(
        `[${traceId}] Successfully added CSV: ${targetPath} (${errors.length} records)`,
      );
      return true;
    } catch (error) {
      this.logger.error(
        `[${traceId}] Unexpected error in addCSVToZip for project '${projectId}' date '${date}':`,
        error,
      );
      return false;
    }
  ...
  private async generateCSVContent(
    ...
    this.logger.log(
      `[${traceId}] Generating CSV content for ${errors.length} errors using temp file: ${tempFile}`,
    );
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Failed to create temp directory ${tempDir}: ${mkdirError instanceof Error ? mkdirError.message : String(mkdirError)}`,
        );
      }
      ...
      try {
        ...
        this.logger.log(
          `[${traceId}] Successfully wrote ${formattedErrors.length} records to temp CSV file`,
        );
      } catch (writeError) {
        throw new Error(
          `Failed to write CSV records: ${writeError instanceof Error ? writeError.message : String(writeError)}`,
        );
      }
      ...
      try {
        ...
        throw new Error(
          `Failed to read CSV file: ${readError instanceof Error ? readError.message : String(readError)}`,
        );
      }
      ...
      try {
        ...
        this.logger.warn(
          `[${traceId}] Failed to clean up temp file ${tempFile}:`,
          unlinkError,
        );
      ...
      this.logger.log(
        `[${traceId}] CSV content generated successfully (${csvContent.length} characters)`,
      );
      return csvContent;
    } catch (error) {
      ...
      throw new Error(
        `CSV content generation failed: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
}
