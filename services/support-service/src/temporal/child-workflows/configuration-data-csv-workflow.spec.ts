...
describe('ConfigurationDataCsvGeneratorWorkflow', () => {
  ...
  describe('Workflow Execution', () => {
    ...
    it('should propagate errors from generateConfigurationDataCsv activity', async () => {
      const mockError = new Error('Failed to generate worker CSV');
      const mockGenerateConfigurationDataCsv = jest
        .fn()
      ...
      const worker = await Worker.create({
        connection: testEnv.nativeConnection,
        taskQueue: 'test-queue-error-worker',
        workflowsPath: require.resolve('./configuration-data-csv-workflow'),
        activities: {
          generateConfigurationDataCsv: mockGenerateConfigurationDataCsv,
          generateConfigurationJobCsv: mockGenerateConfigurationJobCsv,
      ...
      const handle = await client.workflow.start(
        ConfigurationDataCsvGeneratorWorkflow,
        {
          args: [{ traceId: 'test-trace-error', payload }],
          taskQueue: 'test-queue-error-worker',
      ...
      await expect(handle.result()).rejects.toThrow(
        'Failed to generate worker CSV',
      );
      expect(mockGenerateConfigurationDataCsv).toHaveBeenCalledWith({
        traceId: 'test-trace-error',
        payload,
      });
      expect(mockGenerateConfigurationJobCsv).not.toHaveBeenCalled();

      worker.shutdown();
      await runPromise;
    });

    it('should propagate errors from generateConfigurationJobCsv activity', async () => {
      const mockError = new Error('Failed to generate job config CSV');
      const mockGenerateConfigurationDataCsv = jest
        .fn()
        .mockResolvedValue('Worker CSV generated successfully');
      const mockGenerateConfigurationJobCsv = jest
      ...
      const worker = await Worker.create({
        connection: testEnv.nativeConnection,
        taskQueue: 'test-queue-error-job',
        workflowsPath: require.resolve('./configuration-data-csv-workflow'),
        activities: {
          generateConfigurationDataCsv: mockGenerateConfigurationDataCsv,
          generateConfigurationJobCsv: mockGenerateConfigurationJobCsv,
      ...
      const handle = await client.workflow.start(
        ConfigurationDataCsvGeneratorWorkflow,
        {
          args: [{ traceId: 'test-trace-job-error', payload }],
          taskQueue: 'test-queue-error-job',
          workflowId: 'test-workflow-job-error',
        },
      );

      await expect(handle.result()).rejects.toThrow(
        'Failed to generate job config CSV',
      );
      expect(mockGenerateConfigurationDataCsv).toHaveBeenCalledWith({
        traceId: 'test-trace-job-error',
        payload,
      });
      expect(mockGenerateConfigurationJobCsv).toHaveBeenCalledWith({
        traceId: 'test-trace-job-error',
