...
export class RedisConsumerService implements OnModuleDestroy {
    ...
    private activeWorkers: Map<string, { startedAt: number; lastHeartbeatAt: number; workerRef: Worker | null }> = new Map();
    private terminatedWorkerIds: Set<string> = new Set(); // jobIds intentionally terminated by the cron hung-detection logic
            ...
            for (const key of keys) {
                ...
                if (Object.values(consumerStatuses).some(status => status === 'active')) {
                    if (this.activeWorkers.has(jobId)) {
                        ...
                        if (idleMs > this.workerHeartbeatTimeoutMs) {
                            ...
                            if (tracked.workerRef) {
                                this.terminatedWorkerIds.add(jobId);
                    ...
                    this.createConsumerWorkerThread(jobId)
                        ...
                        .catch(error => {
                            this.activeWorkers.delete(jobId);
                            if (this.terminatedWorkerIds.has(jobId)) {
                                this.terminatedWorkerIds.delete(jobId);
    ...
    private async processFileDataInBatches(
        ...
        if (context.records.length >= this.batchSize) {
            ...
            let flushSucceeded: boolean = await this.flushInventory(jobRunId, jobContext);
    ...
    private async flushInventory(jobRunId: string, jobContext: JobManagerContext): Promise<boolean> {
        ...
        let redisUpdated: boolean = false;
