...
export abstract class StorageClient {
  ...
  /**
   * Normalize a certificate field that may be a string or string array to a single string.
   * Node.js tls.PeerCertificate fields can be string | string[] when multiple values exist.
   */
  protected normalizeCertField(value: string | string[] | undefined): string | undefined {
  ...
  protected extractIssuerChain(cert: any): any[] {
    ...
    while (current && current.issuerCertificate && current !== current.issuerCertificate) {
      chain.push({
        CN: this.normalizeCertField(current.issuer?.CN),
        O: this.normalizeCertField(current.issuer?.O),
        OU: this.normalizeCertField(current.issuer?.OU),
        C: this.normalizeCertField(current.issuer?.C),
        ST: this.normalizeCertField(current.issuer?.ST),
        L: this.normalizeCertField(current.issuer?.L),
      });
