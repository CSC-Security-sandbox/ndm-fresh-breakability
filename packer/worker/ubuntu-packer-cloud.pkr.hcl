...
source "azure-arm" "azure_ubuntu" {
  ...
  managed_image_name = "${var.project_name}-worker-${local.formatted_timestamp}"
  ...
  azure_tags = {
    "StackName" = "${var.project_name}-worker-ami-${local.formatted_timestamp}"
    "CreatedBy" = "Packer"
...
source "amazon-ebs" "aws_ubuntu" {
  ...
  ami_name      = "${var.project_name}-worker-${local.formatted_timestamp}"
